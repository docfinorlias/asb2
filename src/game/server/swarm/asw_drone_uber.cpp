...
{	
	...
	//SetModel( "models/swarm/drone/drone.mdl" );
	//если менять модель то нету ACT_ALIEN_FLINCH_SMALL и т.п. надо заменить на что-то, т.к. у модели нет такого дерьма
	...
	//Zak - размер убера (по-умолчанию 1.3)
	float scale = 1.3;
	UTIL_SetSize(this, Vector(-17,-17,0)*scale, Vector(17,17,69)*scale);
...
void CASW_Drone_Uber::Precache( void )
{
	...
	//PrecacheModel( "models/swarm/drone/drone.mdl" );
