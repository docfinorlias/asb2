...
ConVar asw_marine_death_protection("asw_marine_death_protection", "1", FCVAR_NONE, "Prevents marines from dying in one hit, unless on 1 health");
...
ConVar asw_marine_ff("asw_marine_ff", "1", FCVAR_NONE, "Marine friendly fire setting (0 = FFGuard, 1 = Normal (based on mission difficulty), 2 = Always max)", true, 0, true, 2);
ConVar asw_marine_ff_guard_time("asw_marine_ff_guard_time", "5.0", FCVAR_NONE, "Amount of time firing is disabled for when activating friendly fire guard");
ConVar asw_marine_ff_dmg_base("asw_marine_ff_dmg_base", "1.0", FCVAR_NONE, "Amount of friendly fire damage on mission difficulty 5");
...
ConVar asw_marine_burn_time_easy("asw_marine_burn_time_easy", "6", FCVAR_NONE, "Amount of time marine burns for when ignited on easy difficulty");
ConVar asw_marine_burn_time_normal("asw_marine_burn_time_normal", "8", FCVAR_NONE, "Amount of time marine burns for when ignited on normal difficulty");
ConVar asw_marine_burn_time_hard("asw_marine_burn_time_hard", "12", FCVAR_NONE, "Amount of time marine burns for when ignited on hard difficulty");
ConVar asw_marine_burn_time_insane("asw_marine_burn_time_insane", "15", FCVAR_NONE, "Amount of time marine burns for when ignited on insane difficulty");
ConVar asw_marine_time_until_ignite("asw_marine_time_until_ignite", "0.7f", FCVAR_NONE, "Amount of time before a marine ignites from taking repeated burn damage");
ConVar asw_marine_ff_ai("asw_marine_ff_ai", "1", FCVAR_CHEAT, "Enable friendly fire by bots");
...
int CASW_Marine::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( bFriendlyFire )
	{
		CASW_Marine *pOtherMarine = dynamic_cast<CASW_Marine*>(newInfo.GetAttacker()); //zak - pOtherMarine тот, кто наносит удары
		...
		if ( pOtherMarine && !asw_marine_ff_ai.GetBool() && !pOtherMarine->IsInhabited() && !(newInfo.GetDamageType() & DMG_DIRECT) )
		{
...
void CASW_Marine::HurtAlien(CBaseEntity *pAlien, const CTakeDamageInfo &info)
{
	...
	//zak - used tesla gun on any mobs
	if (info.GetWeapon() && FClassnameIs( info.GetWeapon(), "asw_weapon_tesla_gun" )) {
		CASW_Marine_Resource *pMR = GetMarineResource();
		if (pMR && !pMR->m_bTeslaGunUsed) {
			pMR->m_bTeslaGunUsed = true;
		}
	}
...
void CASW_Marine::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( ASWGameRules() )
	{
		...
		//=====Zak убрал камеру смерти нафих, вне зависимости от настроек
	...
	if (info.GetAttacker() && info.GetAttacker()->Classify() == CLASS_ASW_MARINE )
	{
		...
		if ( pOtherMarine && GetMarineProfile() && pOtherMarine->GetMarineProfile() )
		{
			...
			if ( pMR )
			{
				...
				if ( pOtherMarine == this ) // killed himself
				{
					if ( GetMarineProfile()->m_bFemale )
						UTIL_ClientPrintAll( ASW_HUD_PRINTTALKANDCONSOLE, "#asw_suicide_female", szName );
						//UTIL_ClientPrintAll( ASW_HUD_PRINTTALKANDCONSOLE, "#asw_suicide_female", GetCommander()->GetPlayerName() );
					else
						UTIL_ClientPrintAll( ASW_HUD_PRINTTALKANDCONSOLE, "#asw_suicide_male", GetMarineProfile()->m_ShortName ); 
						//UTIL_ClientPrintAll( ASW_HUD_PRINTTALKANDCONSOLE, "#asw_suicide_male", GetCommander()->GetPlayerName() );
					pMR->m_iSuicide++;
				}
				else
				{
					CASW_Marine_Resource *pMROther = pOtherMarine->GetMarineResource();
					if ( pMROther )// если убил своего (даже бота)
					{
						...
						//zak считаем количество убийств своих
						pMROther->m_iKiller++;
					}
				}
			}
	...
	//===== Zak ѕишем сообщение, если марин погиб
	else
	{
		CFmtStr str; //ASW_HUD_PRINTTALKANDCONSOLE
		UTIL_ClientPrintAll( HUD_PRINTTALK, str.sprintf( "Dude %s has died\n", GetCommander()->GetPlayerName() ) );
	}
	//=====
...
{ 
	...
	//Zak - горим (сгораем) быстрее
	float flBurnTime = ( asw_marine_ff_absorption.GetInt() > 0 ) ? asw_marine_time_until_ignite.GetFloat() : MIN(0.2f, asw_marine_time_until_ignite.GetFloat());
