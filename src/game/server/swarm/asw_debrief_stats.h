...
public:
	...
	//zak - огнемет/фламер flamer (get flamers shots)
	int GetFlamerShots(int iMarineIndex) { return m_iFlamerShots[iMarineIndex]; }
	int GetFlamerHit(int iMarineIndex) { return m_iFlamerHit[iMarineIndex]; }
	...
	//zak: фламер/огнемет flamers hit
	CNetworkArray( int,		m_iFlamerShots,		ASW_MAX_MARINE_RESOURCES );
	CNetworkArray( int,		m_iFlamerHit,		ASW_MAX_MARINE_RESOURCES );
