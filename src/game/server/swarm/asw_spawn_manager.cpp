...
ConVar asw_batch_interval("asw_batch_interval", "3", FCVAR_CHEAT, "Time between successive batches spawning in the same spot");
...
#define WANDERER_QUEUE_SIZE 25
const char **wandererQueue = new const char *[WANDERER_QUEUE_SIZE];
int wandererQueueStart = 0;
int wandererQueueEnd = 0;
#define MINIMUM_SPAWN_DELAY 0.050f


inline int wandererSize() {
	return (wandererQueueEnd-wandererQueueStart+WANDERER_QUEUE_SIZE)%WANDERER_QUEUE_SIZE;
}
...
float CASW_Spawn_Manager::getWandererDifficulty() {
	if (!spawnSelection.GetCurrentSpawnSet()) {
		Msg("*** no spawn set\n");
		return 0;
	}

	int minSize = spawnSelection.GetCurrentSpawnSet()->GetMinWandererSize();
	int maxSize = spawnSelection.GetCurrentSpawnSet()->GetMaxWandererSize();
	float avgSize = (maxSize+minSize)/2;
	...
	float avgPeakTime = (asw_director_peak_min_time.GetFloat() + asw_director_peak_max_time.GetFloat())/2;
	float avgRelaxedTime = (asw_director_relaxed_min_time.GetFloat() + asw_director_relaxed_max_time.GetFloat())/2;
	float avgTime = (avgPeakTime + avgRelaxedTime) / avgPeakTime;

	float avg = avgSize / (avgTime / 8.5);
	int spawnType = SpawnTypeFromString("WANDERER");
	
	float diffUnscaled = avg * spawnSelection.GetCurrentSpawnSet()->getEstimatedDifficulty(spawnType);
	DevMsg(9, "unscaled wanderer difficulty: %.3f\n", diffUnscaled);
	...
	if (Q_stricmp(mapName, "ASI-Jac1-LandingBay_02") == 0)	mapScale = 10.8;
	if (Q_stricmp(mapName, "ASI-Jac2-Deima") == 0)			mapScale =  3.3;
	if (Q_stricmp(mapName, "ASI-Jac3-Rydberg") == 0)		mapScale =  5.0;
	if (Q_stricmp(mapName, "ASI-Jac4-Residential") == 0)	mapScale =  3.2;
	if (Q_stricmp(mapName, "ASI-Jac6-SewerJunction") == 0)	mapScale = 1; //6.9
	if (Q_stricmp(mapName, "ASI-Jac7-TimorStation") == 0)	mapScale =  4.4;
	//paranoia
	if (Q_stricmp(mapName, "mission1") == 0)				mapScale = 10.3;
	if (Q_stricmp(mapName, "as_paranoia1") == 0)			mapScale = 10.3;
	
	if (Q_stricmp(mapName, "mission2") == 0)				mapScale =  1.7;
	if (Q_stricmp(mapName, "as_paranoia2") == 0)			mapScale =  1.7;
	*/
	
	char buffer[32];
	Q_snprintf(buffer, sizeof(buffer), "%.1f", diffUnscaled);
	//float mapScale = ("%.1f", diffUnscaled);
	float mapScale = atof(buffer);
	DevMsg(9, "scaled wanderer difficulty: %.1f\n", mapScale);
	//Msg("****** Unscaled: %f  Scaled: %f  Return: %f  \n", diffUnscaled, mapScale, diffUnscaled / mapScale);
	return diffUnscaled / mapScale;
}


void CASW_Spawn_Manager::queueWanderers() {
	int min = spawnSelection.GetCurrentSpawnSet()->GetMinWandererSize();
	int max = spawnSelection.GetCurrentSpawnSet()->GetMaxWandererSize();
	int count = RandomInt(min, max);

	int spawnType = SpawnTypeFromString("WANDERER");
	for (int i=0;i<count;i++) {
		CASW_Spawn_Definition* spawn = spawnSelection.GetCurrentSpawnSet()->GetSpawnDef(spawnType);
		CASW_Entry* entry = spawn->GetEntry(0);
		const char* alien = entry->GetNPCClassname();

		wandererQueue[wandererQueueEnd++] = alien;
		wandererQueueEnd %= WANDERER_QUEUE_SIZE;
	}
}
...
const Vector CASW_Spawn_Manager::shiftSpawnPosition(const Vector vecSpawnPos, const Vector vecMins, const Vector vecMaxs, float extraClearanceFactor) {
	float clear_p__ = traceHull(vecSpawnPos, vecMins, vecMaxs, extraClearanceFactor, 0);
	float clear_m__ = traceHull(vecSpawnPos, vecMins, vecMaxs, -extraClearanceFactor, 0);
	if (clear_p__ != 1 && clear_m__ != 1) {
		return Vector(0,0,-1);
	}


	float clear___p = traceHull(vecSpawnPos, vecMins, vecMaxs, 0, extraClearanceFactor);
	float clear___m = traceHull(vecSpawnPos, vecMins, vecMaxs, 0, -extraClearanceFactor);
	if (clear___p != 1 && clear___m != 1) {
		return Vector(0,0,-1);
	}

	//shift in the opposite direction of the colision by (1-fractionToCollision)*maxSize
	float clear_x = - ((1.1-clear_p__) + -(1.1-clear_m__));
	float clear_y = - ((1.1-clear___p) + -(1.1-clear___m));
	
	return Vector(vecMaxs.x*clear_x,vecMaxs.y*clear_y,0);
}

float CASW_Spawn_Manager::traceHull(const Vector vecSpawnPos, const Vector vecMins, const Vector vecMaxs, int x, int y) {
	trace_t tr;
	UTIL_TraceHull( vecSpawnPos,
		vecSpawnPos + Vector( vecMaxs.x * x, vecMaxs.y * y, 0 ),
		vecMins,
		vecMaxs,
		MASK_NPCSOLID,
		NULL,
...
bool CASW_Spawn_Manager::GetAlienBounds( const char *szAlienClass, Vector &vecMins, Vector &vecMaxs )
{
	...
	if (strcmp(szAlienClass, "asw_drone_uber") == 0) {
...
{	
	...
	if (strcmp("asw_buzzer", szAlienClass) != 0) {
...
bool CASW_Spawn_Manager::SpawnRandomQueen()
{
	int iNumNodes = g_pBigAINet->NumNodes();
	if ( iNumNodes < 6 )
		return false;
	...
	if ( pBestArea )
	{
		CBaseEntity *pAlien = SpawnAlienAt( "asw_queen", pBestArea->m_pNode->GetPosition( nHull ), RandomAngle( 0, 360 ) );
		IASW_Spawnable_NPC *pSpawnable = dynamic_cast<IASW_Spawnable_NPC*>( pAlien );
		if ( pSpawnable )
		{
			pSpawnable->SetAlienOrders(AOT_SpreadThenHibernate, vec3_origin, NULL);
