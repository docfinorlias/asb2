...
void CASW_Parasite::Event_Killed( const CTakeDamageInfo &info )
{
	...
	//zak - убиваем паразита пилой
	if ( !m_bDefanged && !m_bDoEggIdle.Get() && info.GetAttacker() && info.GetWeapon() && info.GetAttacker()->Classify() == CLASS_ASW_MARINE ) //если убил паразита любым оружием или чем-нибудь
	{
		CASW_Marine *pMarine = static_cast<CASW_Marine*>( info.GetAttacker() );
		if ( pMarine && pMarine->IsInhabited() && pMarine->GetCommander() )
		{
			if ( pMarine->GetMarineResource() ) {
				//Msg("*******ParaSystem::Alien killed by a %s\n", info.GetWeapon()->GetClassname());
				if (FClassnameIs( info.GetWeapon(), "asw_weapon_chainsaw" )) { //если убили паразита пилой
					if ( pMarine->GetMarineResource() ) {
						pMarine->GetMarineResource()->m_iParasitesKilledByChainsaw++;
					}
				}
			}
		}
