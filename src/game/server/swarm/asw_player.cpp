...
CASW_Player::CASW_Player()
{
	...
	//zak
	m_bAllreadyWelcomed = false;
	m_bSentJoinedPoints = false;
	m_iPointsOnJoined = 0;
...
unsigned int __stdcall GetPlayerScore (void* pParam)
{
	tScores* pScore = (tScores*) pParam;
	//int *pIdx = (int *) pParam;

	char buffer[1024];
	int pPoints = 0;

	CASW_Player *pPlayer = (CASW_Player *)UTIL_PlayerByIndex(pScores->idx);
	if (!pPlayer) {
		Msg("***Incorrect get points. Retrun 0 pts.\n");
		return 0;
	}
	...
	if ( !myBase->cInit() ) {
		...
		Q_snprintf(buffer, sizeof(buffer), "SELECT sum(pts) AS pPoints FROM (SELECT sum(points) AS pts FROM base_stat ta WHERE ta.user_steam_id='%s' UNION SELECT sum(ach_points) AS pts FROM ach_stat ta WHERE ta.user_steam_id='%s') AS pPoint", pScore->steamid, pScore->steamid); //CASW_Player->GetASWNetworkID()
	...
	//CASW_Player *newPlayer = (CASW_Player *) pParam;
	CASW_Player *newPlayer = (CASW_Player *)UTIL_PlayerByIndex(pScores->idx);
	if (!newPlayer)
		return 0;
	newPlayer->m_iPointsOnJoined = pPoints;
	//pPlayer->m_iPointsOnJoined = pPoints;
	return 0;
}
	...
	else if ( FStrEq( pcmd, "cl_fullyjoined") )
	{
		if (!m_bSentJoinedMessage)
		{
			if (gpGlobals->maxClients > 1)
			{
				...
				pScores = new tScores;
				pScores->idx = entindex();
				strcpy (pScores->steamid, GetASWNetworkID());
				(HANDLE)_beginthreadex(NULL, 0, GetPlayerScore, pScores, 0, NULL);
