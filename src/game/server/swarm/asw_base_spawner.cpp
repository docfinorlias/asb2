...
IASW_Spawnable_NPC* CASW_Base_Spawner::SpawnAlien( const char *szAlienClassName, const Vector &vecHullMins, const Vector &vecHullMaxs )
{
	...
	//=====0008-hacked-asw_spawner-spawn-an-uber-drone-when-the-name.patch
	//CBaseEntity	*pEntity = CreateEntityByName( szAlienClassName );
	CBaseEntity	*pEntity;
	if (Q_stricmp(szAlienClassName, "asw_drone") == 0 && Q_stricmp(m_AlienName.ToCStr(), "asw_drone_uber") == 0) {
		pEntity = CreateEntityByName( "asw_drone_uber" );
	} else {
		pEntity = CreateEntityByName( szAlienClassName );
	}
	//=====
