	...
	{	
		...
		if (bFastHack && pMarine && pMarine->GetMarineResource())
		{
			...
			if ( pMarine->IsInhabited() && pMarine->GetCommander() )
			{
				...
				//zak - noadren fasthack door
				if ( endadren > gpGlobals->curtime ) { //если находимся под адреном
					//pMarine->GetMarineResource()->m_iAdrenFastDoorHacks++; //считаем адреновые фастхаки
				} else
					pMarine->GetMarineResource()->m_iNoAdrenFastDoorHacks++; //считаем безадреновые фастхаки
			}
		}
