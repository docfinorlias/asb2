...
bool CASW_Game_Resource::AddMarineResource( CASW_Marine_Resource *m, CASW_Player *pPlayer, int nPreferredSlot )
{
	//zak - max players in game, other observers
	int maxClientsForSelect = 4;
	...
	if ( nPreferredSlot != -1 )
	{
		...
		for (int k=0;k<maxClientsForSelect;k++)
		{
	...
	for (int i=0;i<maxClientsForSelect;i++)
	{
	...
	UTIL_SayText("At current moment you are only observer", pPlayer);
...
void CASW_Game_Resource::SetLeader(CASW_Player *pPlayer)
{
	...
	/*
	//авто галку в студию ВСЕМ!!!
	for ( int i = 1; i <= gpGlobals->maxClients; i++ ) {
		//CASW_Player* pPlay = dynamic_cast<CASW_Player*>(UTIL_PlayerByIndex(i));
