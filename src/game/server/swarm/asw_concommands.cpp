...
ConCommand bi_update_svtags("bi_update_svtags", bi_update_svtags_f, "Forces an update of sv_tags to reflect changes in a config.", 0);

void MapInfoF( const CCommand& args )
{
	...
	char buffer[128];
	Q_snprintf(buffer, sizeof(buffer), "======================\nMission index: %s\nCampaign index: %s\n======================\n", mapName, pCampaign->m_szCampaignFilename);
	Msg (buffer); ClientPrint(pPlayer, HUD_PRINTTALK, buffer);
	Q_snprintf(buffer, sizeof(buffer), "Mission name : %s \n", pCampaign->GetMissionByMapName(mapName)->m_MissionName);
	Msg (buffer); ClientPrint(pPlayer, HUD_PRINTTALK, buffer);
	Q_snprintf(buffer, sizeof(buffer), "Campaign name: %s\n", pCampaign->m_CampaignName);
	Msg (buffer); ClientPrint(pPlayer, HUD_PRINTTALK, buffer);
	Q_snprintf(buffer, sizeof(buffer), "Total maps in campaign: %d\n", pCampaign->GetNumMissions()-1);
	Msg (buffer); ClientPrint(pPlayer, HUD_PRINTTALK, buffer);
}
static ConCommand MapInfo("mapinfo", MapInfoF, "Map info", 0);

void imleadF( const CCommand& args )
{
	CASW_Player* pPlayer = ToASW_Player( UTIL_GetCommandClient() );
	if (!pPlayer)
		return;
...
static ConCommand ImLead("imlead", imleadF, "Set me leader now!", 0);

void verF( const CCommand& args )
{
	CASW_Player* pPlayer = ToASW_Player( UTIL_GetCommandClient() );
	if (!pPlayer)
		return;
	...
	char buffer[25];
	Q_snprintf(buffer, sizeof(buffer), "MOD version: %.1f\n", (version-21)/10);
	ClientPrint(pPlayer, HUD_PRINTTALK, buffer);
	UTIL_ShowMessage("VER.", pPlayer);
}
static ConCommand ver("ver", verF, "Get MOD version", 0);
