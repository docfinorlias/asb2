...
BEGIN_DATADESC( CASW_Marine_Resource )
	...
	//zak : фламер/огнемет flamer
	DEFINE_FIELD( m_iFlamerFired, FIELD_INTEGER ),

	//использование теслы
	DEFINE_FIELD( m_bTeslaGunUsed, FIELD_BOOLEAN ),
	...
	//zak - если добавить еще парочку тройку переменных с INTEGER
	//то это будет считаться ошибкой, т.к. очень мало памяти выделенно под назначение переменных
	//если будет переполнение памяти, то некорретно будут считаться фастхаки и возможно еще что-то
	DEFINE_FIELD( m_iKiller, FIELD_SHORT ),
	DEFINE_FIELD( m_iSuicide, FIELD_SHORT ),
	DEFINE_FIELD( m_iNoAdrenFastDoorHacks, FIELD_SHORT ),
	DEFINE_FIELD( m_iParasitesKilledByChainsaw, FIELD_SHORT ),
...
void CASW_Marine_Resource::UsedWeapon(CASW_Weapon *pWeapon, int iShots)
{
	...
	if (GetMarineEntity())
	{
		...
		if (gpGlobals->curtime <= GetMarineEntity()->m_fLastShotAlienTime + 1.0f)	// give a 1 second window after hitting an alien to count shots as hitting
		{
			...
			// Закомментил в связи с тем, что вроде как жестоко сильно по отношению к игроку при использовании флеймера
			//Zak ===== Умираем, если использовали фламер на мобах (исключение бочки, биомасса и т.п.)
			//или просто прибавляем количество выстрелов онгеметом
			
			
			CBaseCombatWeapon *pMyWeapon = GetMarineEntity()->GetActiveWeapon();


			if ( pMyWeapon && FClassnameIs( pMyWeapon, "asw_weapon_flamer" ) ) { // если использовал (заюзал) огнемет
				if (m_iFlamerFired <= 0){
					char buffer[128];
					Q_snprintf(buffer, sizeof(buffer), "%s: flamerthrower detected!\n", GetMarineEntity()->GetPlayerName() );
					UTIL_ClientPrintAll( HUD_PRINTTALK,  buffer);
				}
				m_iFlamerFired += iShots;
				 
			
				
				//CTakeDamageInfo info(NULL, NULL, Vector(0,0,0), GetAbsOrigin(), 2500.0f, DMG_BURN); //full death
				//GetMarineEntity()->m_iHealth -= info.GetDamage();

				//Сгораем сами если заюзали фламер на мобах (только здесь правильное решение, в других файлах не то)
				//GetMarineEntity()->ASW_Ignite(0.1f,0, GetMarineEntity(), pMyWeapon);

				/*
				if ( GetMarineEntity()->m_iHealth <= 0 ) {
					// так как вроде мы поапали то минус один выстрел
					m_iShotsFired -= iShots;
					
					CFmtStr str; //ASW_HUD_PRINTTALKANDCONSOLE
					UTIL_ClientPrintAll( HUD_PRINTTALK, str.sprintf( "NOOB %s used a flamerthrower and for it he has died\n", GetMarineEntity()->GetPlayerName() ) );
					GetMarineEntity()->Event_Killed( info );
					//Если вдруг захочу чтоб сгорал то надо подумать над этим (взято из asw_fire.cpp)
			...
			if (GetMarineEntity()->IsInhabited()) { //если стреляет игрок. и попал в моба
