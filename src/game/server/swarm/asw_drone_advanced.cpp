...
ConVar asw_drone_acceleration("asw_drone_acceleration", "5", FCVAR_NONE, "How fast the swarm drone accelerates, as a multiplier on his ideal speed");
...
bool CASW_Drone_Advanced::MoveExecute_Alive(float flInterval)
{
	...
	if (GetTask() && (GetTask()->iTask == TASK_MELEE_ATTACK1))		// do a non plane sliding movement for attacking
	{
		...
		trace_t trace;
		UTIL_TraceEntity( this, start, end, MASK_NPCSOLID, &trace );

		if (trace.fraction != 1) {
			//this move would have moved us through a solid object - dont do it
			SetAbsOrigin(start);
		}
	}
