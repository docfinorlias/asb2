...
float CServerGameDLL::getDifficulty(char* textRepresentation, int textRepresentationLength) {
	extern ConVar asw_skill;
	int skill = asw_skill.GetInt();

	int missionDifficulty = 5;
	if ( ASWGameRules()) {
		missionDifficulty = ASWGameRules()->GetMissionDifficulty();
	}

	int defaultMissionDifficulty;
	switch(skill) {
		case 1: defaultMissionDifficulty =  3; break;
		case 2: defaultMissionDifficulty =  5; break;
		case 3: defaultMissionDifficulty =  7; break;
		case 4: defaultMissionDifficulty = 10; break;
		case 5: defaultMissionDifficulty = 13; break;
		
		default: Msg("unknown difficulty");
			defaultMissionDifficulty = 3*skill-2;
	}

	if (abs(missionDifficulty-defaultMissionDifficulty) > 2) {
		missionDifficulty = defaultMissionDifficulty;
	}

	DevMsg(5, "Mission difficulty: %d\n", missionDifficulty);

	// alien health
	//		1/x for under 1
	//		1 points for every point over 2 (-1 for 1, 0 for 2, 1 for 3)
	float alienHealthDiff = 0;
	{
		extern ConVar asw_difficulty_alien_health_step;

		float health = 1 + asw_difficulty_alien_health_step.GetFloat() * (missionDifficulty-5);

		if (health < 1) {
			alienHealthDiff = -(1/(health + 0.0001));
			DevMsg(5, "alienHealthDiff1a = %f\n", alienHealthDiff);
		} else if (health > 1) {
			alienHealthDiff = health - 2;
			DevMsg(5, "alienHealthDiff1a = %f\n", alienHealthDiff);
		}
	}
	...
	// alien damage
	//		1/x for under 1
	//		0.7 points for every point over 2 (-0.7 for 1, 0 for 2, 0.7 for 3)
	float alienDamageDiff = 0;
	{
		extern ConVar asw_difficulty_alien_damage_step;

		float damage = 1 + asw_difficulty_alien_damage_step.GetFloat() * (missionDifficulty-5);

		if (damage < 1) {
			alienDamageDiff = -(1/(damage + 0.0001)-0.3);
			DevMsg(5, "alienDamageDiff1a = %f\n", alienDamageDiff);
		} else if (damage > 1) {
			alienDamageDiff = (damage - 2) * 0.7;
			DevMsg(5, "alienDamageDiff1a = %f\n", alienDamageDiff);
		}
	}
	...
	// alien speed
	//		1/x+0.83 for 0 to 1.2
	//		2 point for every 0.6 after that
	float alienSpeedDiff = 0;
	{
		float alienSpeed = 1;
		switch (skill) {
		case 1: extern ConVar asw_alien_speed_scale_easy;
			alienSpeed = asw_alien_speed_scale_easy.GetFloat();
			break;
		case 2: extern ConVar asw_alien_speed_scale_normal;
			alienSpeed = asw_alien_speed_scale_normal.GetFloat();
			break;
		case 3: extern ConVar asw_alien_speed_scale_hard;
			alienSpeed = asw_alien_speed_scale_hard.GetFloat();
			break;
		case 4: case 5: extern ConVar asw_alien_speed_scale_insane;
			alienSpeed = asw_alien_speed_scale_insane.GetFloat();
			break;
		default: DevMsg(5, "unsupported asw_skill for difficulty calculation: %f\n", skill);
		}

		if (alienSpeed < 1.2) {
			alienSpeedDiff = -1/(alienSpeed + 0.0001)+0.83;
			DevMsg(5, "alienSpeedDiff1a = %f\n", alienSpeedDiff);
		} else if (alienSpeed > 1.2) {
			alienSpeedDiff = (alienSpeed-1.2)*(3+1./3);
			DevMsg(5, "alienSpeedDiff1b = %f\n", alienSpeedDiff);
		}
	}

	// marine speed
	//		1 at 0speed, 0 at 1speed, -1 at 2 speed
	float marineSpeedDiff = 0;
	{
		float marineSpeed = 1;
		switch (skill) {
		case 1: extern ConVar asw_marine_speed_scale_easy;
			marineSpeed = asw_marine_speed_scale_easy.GetFloat();
			break;
		case 2: extern ConVar asw_marine_speed_scale_normal;
			marineSpeed = asw_marine_speed_scale_normal.GetFloat();
			break;
		case 3: extern ConVar asw_marine_speed_scale_hard;
			marineSpeed = asw_marine_speed_scale_hard.GetFloat();
			break;
		case 4: extern ConVar asw_marine_speed_scale_insane;
			marineSpeed = asw_marine_speed_scale_insane.GetFloat();
			break;
		default: DevMsg(5, "unsupported asw_skill for difficulty calculation: %f\n", skill);
		}

		marineSpeedDiff = -(marineSpeed-1);
		DevMsg(5, "marineSpeedDiff = %f\n", marineSpeedDiff);
	}
	...
	//2 (capped) for ff >=3, 0 for ff=1.5, -2 for ff=0
	//0.2 for sg ff
	float friendlyFireDiff = 0;
	{
		extern ConVar asw_marine_ff;
		extern ConVar asw_marine_ff_absorption;

		if (asw_marine_ff_absorption.GetInt() != 0 || asw_marine_ff.GetInt() == 0) {
			friendlyFireDiff = -2;
			DevMsg(5, "friendlyFireDiff1a = %f\n", friendlyFireDiff);
		} else {
			extern ConVar asw_marine_ff_dmg_base;
			extern ConVar asw_marine_ff_dmg_step;

			float ffDmg = asw_marine_ff_dmg_base.GetFloat() + asw_marine_ff_dmg_step.GetFloat() * (missionDifficulty-5);
			ffDmg = MAX(0, ffDmg);
			ffDmg = MIN(3, ffDmg);

			friendlyFireDiff = MIN(2, ffDmg*4/3-2);
			DevMsg(5, "friendlyFireDiff1b = %f\n", friendlyFireDiff);
		}
	}
	{
		extern ConVar asw_sentry_friendly_fire_scale;
		float sgDmg = asw_sentry_friendly_fire_scale.GetFloat();

		friendlyFireDiff += MIN(0.2, sgDmg*0.2);
		DevMsg(5, "friendlyFireDiff2 = %f\n", friendlyFireDiff);
	}


	// 0 to N points - 3 at reasonably high levels (my defaults)
	//		2 points for every 0.333 spawns per second
	float hordeDiff = 0;
	{
		extern ConVar asw_horde_override; 
		if (asw_horde_override.GetInt() == 1) {
			extern ConVar asw_horde_size_max; //30
			extern ConVar asw_horde_size_min; //10
			extern ConVar asw_horde_interval_min; //15
			extern ConVar asw_horde_interval_max; //75

			float averageSpawnTime = (asw_horde_interval_max.GetFloat() + asw_horde_interval_min.GetFloat())/2;
			if (averageSpawnTime > 120) {
				hordeDiff -= 1;
				DevMsg(5, "hordeDiff1 = %f\n", hordeDiff);
			}
			float averageHordeSize = (asw_horde_size_max.GetFloat() + asw_horde_size_min.GetFloat())/2;

			float aliensPerSecond = averageHordeSize / averageSpawnTime;


			hordeDiff += 2 * aliensPerSecond/(1/3.);
			DevMsg(5, "hordeDiff2 += %f\n", hordeDiff);
		}
	}


	// 0 to N points - 3 at reasonably high levels (my defaults)
	//		modified by per map difficulty table
	float wandererDiff = 0;
	{
		extern ConVar asw_wanderer_override;

		if (asw_wanderer_override.GetInt() == 1 && ASWSpawnManager()) {
			wandererDiff += 2 * ASWSpawnManager()->getWandererDifficulty();
			DevMsg(5, "wandererDiff1 = %f\n", wandererDiff);
			//Msg("=====WanderDiff: %f  =====Ready: %f    Wanderers:  %.1f\n", ASWSpawnManager()->getWandererDifficulty(), wandererDiff, wandererDiff);
		}
	}


	// -2(at 0) to 0 (at 15) to 0.2(capped at 20) for marine burn time
	// -2(capped at 1) to 0 (at 0.7) to 0.3(at 0) for marine ignite time
	// -0.5f for each second above 5s alien burn time
	float flamerDiff = 0;
	{
		float marineBurnTime = 15;
		switch (skill) {
		case 1: extern ConVar asw_marine_burn_time_easy;
			marineBurnTime = asw_marine_burn_time_easy.GetFloat();
			break;
		case 2: extern ConVar asw_marine_burn_time_normal;
			marineBurnTime = asw_marine_burn_time_normal.GetFloat();
			break;
		case 3: extern ConVar asw_marine_burn_time_hard;
			marineBurnTime = asw_marine_burn_time_hard.GetFloat();
			break;
		case 4: case 5: extern ConVar asw_marine_burn_time_insane;
			marineBurnTime = asw_marine_burn_time_insane.GetFloat();
			break;
		default: DevMsg(5, "unsupported asw_skill for difficulty calculation: %f\n", skill);
		}
		if (marineBurnTime > 15) {
			flamerDiff += MIN(0.2f, marineBurnTime/25);
			DevMsg(5, "flamerdiff1a = %f\n", flamerDiff);
		} else if (marineBurnTime < 15) {
			flamerDiff += (marineBurnTime-15) / 7.5f;
			DevMsg(5, "flamerdiff1b = %f\n", flamerDiff);
		}
	}
	{
		extern ConVar asw_marine_time_until_ignite;
		float igniteTime = asw_marine_time_until_ignite.GetFloat();
		if (igniteTime < 0.7f) {
			flamerDiff += 0.3f*((0.7f-igniteTime)/0.7f);
			DevMsg(5, "flamerdiff2a = %f\n", flamerDiff);
		} else if (igniteTime > 0.7f) {
			flamerDiff += (4+2.f/3.f)*((0.7f-igniteTime)/0.7f);
			DevMsg(5, "flamerdiff2b = %f\n", flamerDiff);
		}
	}
	{
		extern ConVar asw_alien_burn_duration;
		float burnTime = asw_alien_burn_duration.GetFloat();
		if (burnTime > 5) {
			flamerDiff += -0.5f*(burnTime-5);
			DevMsg(5, "flamerdiff3 = %f\n", flamerDiff);
		}
	}


	// 0 for easy-hard, 0.5 for insane (more spawns on the maps in insane)
	//		0.5 for each difficulty above insane - maybe brutal will have more spawns still? ;)
	float skillDiff = skill<=3?0:0.5*(skill-3);
	DevMsg(5, "skillDiff += %f\n", skill<=3?0:0.5*(skill-3));


	float difficulty = alienHealthDiff + alienDamageDiff + alienSpeedDiff + marineSpeedDiff + godModeDiff + friendlyFireDiff + hordeDiff + wandererDiff + flamerDiff + skillDiff;
	DevMsg(5, "DIFFICULTY: %f\n", difficulty);

	if (textRepresentation != NULL && textRepresentationLength != 0) {
		Q_snprintf(textRepresentation, textRepresentationLength-1, 
			"AlienHealth:    %.1f\nAlienDamage:    %.1f\nAlienSpeed:     %.1f\nMarineSpeed:    %.1f\nGodMode:        %.1f\nFriendlyFire:   %.1f\nHordes:         %.1f\nWanderers:      %.1f\nFlamers:        %.1f\nSkill:          %.1f\n--------------------\nOverall:        %.1f\n",
			alienHealthDiff, alienDamageDiff, alienSpeedDiff, marineSpeedDiff, godModeDiff, friendlyFireDiff, hordeDiff, wandererDiff, flamerDiff, skillDiff, difficulty);
	}
...
void CServerGameDLL::GetMatchmakingTags( char *buf, size_t bufSize )
{
	...
	//Q_snprintf(cText, 50, "Difficulty=%.1f,", fDifficulty);
	//Q_snprintf(cText, 50, "ASB2,");
	Q_strncpy( buf, "ASB2,", bufSize );
	//Q_strncpy( buf, cText, bufSize );
	len = strlen( buf );
	buf += len;
	bufSize -= len;	
