...
public:
	...
	char cSteamId[50];
	char c_IP[24];
}; tPlayerData PlayerData[MAX_PLAYERS+1]; //типа 10 клиентов ))
...
void Host_Say( edict_t *pEdict, const CCommand &args, bool teamonly )
{
	...
	CASW_Player *pInf = NULL;
	if ( pEdict )
	{
		pPlayer = ((CBasePlayer *)CBaseEntity::Instance( pEdict ));
		pInf = ((CASW_Player *)CBaseEntity::Instance( pEdict ));
		Assert( pPlayer );
	...
	//Zak console chat слово консоль
	...
	{
		//zak - убираю двоеточие в соощении консоли
	...
	if ( pPlayer )
	{
		...
		else
		{
			//zak - hook message
			if (pInf) {
				int index = pInf->GetClientIndex()+1;
				

				if (!FStrEq(PlayerData[index].cSteamId, pInf->GetASWNetworkID())) {
					strcpy(PlayerData[index].cSteamId, pInf->GetASWNetworkID());
					PlayerData[index].bStatusTimer = false;
					PlayerData[index].bStatusTimerCenter = false;
					PlayerData[index].bStatusMyKills = false;
					PlayerData[index].bStatusTotalKills = false;
				}
					
				if ( FStrEq( p, "!help" ) ) {
					ClientPrint(pPlayer, HUD_PRINTTALK, "!help - This message\n!timer - Centered timer\n!timer2 - New timer\n!kills - Toggle counter of killed swarms\n");
				} else if ( FStrEq( p, "!timer2" ) ) {
					if (PlayerData[index].bStatusTimer) {
						PlayerData[index].bStatusTimer = false;
						ClientPrint(pPlayer, HUD_PRINTTALK, "Timer is OFF\n");
					} else {
						PlayerData[index].bStatusTimer = true;
						ClientPrint(pPlayer, HUD_PRINTTALK, "Timer is ON\n");
					}
				} else if ( FStrEq( p, "!timer" )) {
					if (PlayerData[index].bStatusTimerCenter) {
						PlayerData[index].bStatusTimerCenter = false;
						ClientPrint(pPlayer, HUD_PRINTTALK, "Centered timer is OFF\n");
					} else {
						PlayerData[index].bStatusTimerCenter = true;
						ClientPrint(pPlayer, HUD_PRINTTALK, "Centered timer is ON\n");
					}
				} else if ( FStrEq( p, "!kills" )) {
					if (PlayerData[index].bStatusMyKills) {
						PlayerData[index].bStatusMyKills = false;
						ClientPrint(pPlayer, HUD_PRINTTALK, "Counter 'kills' is OFF\n");
					} else {
						PlayerData[index].bStatusMyKills = true;
						ClientPrint(pPlayer, HUD_PRINTTALK, "Counter 'kills' is ON\n");
					}
				
				}
				
				else if ( FStrEq( p, "!status" )) {
					//ClientPrint(pPlayer, HUD_PRINTTALK, "This command doesnt working yet.\n");
				}
				
				else UTIL_SayTextFilter( user, text, pPlayer, true );

			} else UTIL_SayTextFilter( user, text, pPlayer, true );

			//p - чистый текст
			//Zak - сообщения от клиента - все сюда пишутся
		}
	}
