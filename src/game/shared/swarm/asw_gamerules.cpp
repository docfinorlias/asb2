...
//check for update server version
float version = 18; // 18 - это версия 1.8
float basever = 0;
ConVar asb2_version ( "asb2_version", "1.8", FCVAR_NOTIFY, "ASB2 version");
//ConVar asb2_version ( "asb2_version", "1.8", FCVAR_NOTIFY);
//ConVar asb2_difficulty ( "asb2_difficulty", "Hard", FCVAR_NOTIFY);
ConVar asb2_difficulty("asb2_difficulty", "Hard", FCVAR_NONE, "ASB2 Difficulty");

//Zak - переменные (my global vars)
bool bStatError = false; //общий флаг проблемы с сервером или связью
bool bNewVersion = false; //новая версия?
bool bLockedMap = true; //по-умолчанию текущая карта заблокирована - статистика отключена

bool CurrentNewView = true;

const char *NOSTAT = "Statistics are currently unavailable!";
...
extern tPlayerData PlayerData[MAX_PLAYERS+1];

//структура-состояние текущей миссии
class tCurrentMap {
public:
	char mapId[60]; //id карты
	char name[200]; //полное наименование миссии(карты)
	int speedrun; //время спидрана карты взятое из базы
	int iSkill; //сложность цифрами (1-5)
	char cSkill[7]; //сложность прописью (Easy, Normal, Hard, Insane, Brutal)
	int ach_count; //всего количество ачивок полученное из базы для данной мисиии
	char campaign_id[50];
	char campaign[100];
	int missions_num;
}; tCurrentMap CurrentMap;
...
public:
	...
	int requirement;
	int current_req;
	bool AchAlreadyInBase;
	bool newline; // true = insert; false = update
} ach_list[200];

class tSaveData {
public:
	short maxClients; //кол-во игроков для обработки статистики
	bool bAllMarinesAlive; //true - если все 4 марина живы, иначе false
	bool bHasAllClasses; //true - если в игре были использованы все классы персонажей
	bool bPerfectTeam; //true - если команда не получила урона
	bool bFlamerUsed; //использовался ли вообще фламер на мобах за миссию. true - ДА
	bool bAwesome; // для ачивки Awesome Team. true - всей командой получено меньше 40% урона. false - хоть у одного больше чем 40% урона (дамага)
	bool bTeslaGunUsed; //использовалась ли тесла. true - ДА
	int iMissionTime; //время затраченное на миссию в секундах
	int iSpeedrunTime; //время спидрана карты взятое из базы
	char cMissionEndTime[80]; //for MySQL timestamp. время окончания мисси. UTC+0
	bool bMissionSuccess; //статус миссии. true - complete, false - failed
	char mapId[60]; //id карты
	char mapName[200]; //полное наименование миссии(карты)
	int iSkill; //сложность цифрами (1-5)
	char cSkill[7]; //сложность прописью (Easy, Normal, Hard, Insane, Brutal)
	int ach_count; //всего количество ачивок полученное из базы для данной мисиии
	int iTotalKills; //количество убитых мобов за миссию. взято из стима
	short iAdrenUsed;
	char campaign_id[50];
	char campaign[100];
	int missions_num;
	...
	class tPlayer {
	public:
		char cSteamId[50]; //steam id
		char cNick[100]; //ник
		bool bAlive; //true если остался жив в конце миссии, иначе false - помер
		bool bPerfect; //true - не ранен
		char cProfile[50]; //класс, кем играет игрок
		float fFriendlyFireDamageDealt;
		float fDamageTaken; //Полученный урон (единицы) (реальный урон от общего количества единиц жизни - например Сарж 140 единиц жизней)
		int iAliensKilled;
		int iMineKills;
		short iKiller;
		short iSuicide;
		int iShieldbugsKilled;
		short iParasitesKilledByChainsaw;

		int iMedicHealing;
		int iCuredInfestation;

		short iNoAdrenFastDoorHacks;
		int iFastDoorHacks;
		int iFastComputerHacks;

		int iParasitesKilled;
		int iSentryKills;
		int iEggKills;
		char c_IP[24];

	} Player[8]; //max players 8 (maxplayers 8)
} *SaveData = 0;
...
//Zak - сообщение в консоли в главном чате игры
void myMessage(const char* str)
{
	char* text = new char[strlen(str) + 6];
	strcpy(text, str);
	strcat(text, "\n");
	Msg (text);
	...
	if (bLockedMap)
		UTIL_ClientPrintAll(HUD_PRINTTALK, "Unknown map!\n");

	UTIL_ClientPrintAll(HUD_PRINTTALK, text);
	delete text;
}

//тут описание всех данных которые я хочу передать в функцию при старте мисси.
struct tBaseData {
	int iSkill;
	char cSkill[7];
	char mapId[60];
...
void ACH_SaveToBase (cSQL *mystat, const char fullmapname[200], const char steamid[50], const char skill[10], const char mapname[100], const char campaign[100], const char campaign_id[50], int achID, char achName[60], int achPoints, char timeTaken[80], const char nick[100], int req = 1, bool upd = false, bool showmsg = true)
{
	char buffer[1536];

	
	if (!upd) {//insert
		Q_snprintf(buffer, sizeof(buffer), "INSERT INTO ach_stat (user_steam_id, difficulty, map_id, campaign, campaign_id, ach_id, ach_points, date_taken, requirement) VALUES ('%s', '%s', '%s', '%s', '%s', %d, %d, '%s', %d)",
			steamid, //SteamID
			skill, // сложность
			mapname,
			campaign,
			campaign_id,
			achID, // текущая обрабатываемая Ачивка ID
			achPoints, // текущие очки назначенные ачивке
			timeTaken, // время получения
			req);
		//Msg("INS: ***%s***\n", steamid);
	} else { //update
		Q_snprintf(buffer, sizeof(buffer), "UPDATE ach_stat SET requirement=%d, date_taken='%s', ach_points=%d WHERE user_steam_id='%s' AND difficulty='%s' AND map_id='%s' AND ach_id=%d",
			req, timeTaken, achPoints, steamid, skill, mapname, achID);
		//Msg("UPD: ***%s***\n", steamid);
	}
	
	//if (mysql_query (&mystat->conn, buffer) != 0) { bStatError = true; }
	if (mysql_query (&mystat->conn, buffer) != 0) {
		bStatError = true;
		Q_snprintf(buffer, sizeof(buffer), "Error: %s, ACH (%s) not saved!", nick, achName);
		myMessage(buffer);
	}

	if (showmsg) {
		if (strcmp("*", skill) == 0)
			Q_snprintf(buffer, sizeof(buffer), "%s achieved: '%s'", nick, achName);
		else if (strcmp("*", mapname) == 0) {
			if (achID == 40) //Veteran отображаем по-другому
				Q_snprintf(buffer, sizeof(buffer), "%s achieved: '%s %s' on %s", nick, campaign, achName, skill);
			else
				Q_snprintf(buffer, sizeof(buffer), "%s achieved: '%s' on %s", nick, achName, skill);
		}
		else
			Q_snprintf(buffer, sizeof(buffer), "%s achieved: '%s' (%s) on %s", nick, achName, fullmapname, skill);
		myMessage(buffer);
	}
}


//void SaveBaseData(void* params)
//unsigned SaveBaseData(void* params)
unsigned __stdcall SaveBaseData(void* params)
{
	if (!bStatError) {
		char buffer[1536];
		tSaveData* par = (tSaveData*) params; // always use a struct!
		cSQL* myBase = new cSQL;

		if ( !myBase->cInit() ) { myMessage(NOSTAT); bStatError = true; } else {
			for ( int i = 1; i <= par->maxClients; i++ ) // цикл по клиентам (маринам)
			{
				//Работа с никами
				//Проверка, если такой ИД с ником уже есть, Если нету, то создаю новую запись
				Q_snprintf(buffer, sizeof(buffer), "SELECT * FROM players WHERE user_steam_id='%s' AND user_name='%s'", par->Player[i].cSteamId, par->Player[i].cNick);
				if(mysql_query(&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
					...
					if (&myBase->res != NULL) {
						if(mysql_num_rows(myBase->res) > 0) { // Если имеется хотя бы одна запись
							//обнуляю все флаги у текущего пользователя
							Q_snprintf(buffer, sizeof(buffer), "UPDATE players SET current=%d WHERE user_steam_id='%s'", 0, par->Player[i].cSteamId);
							if (mysql_query (&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); }
							//устанавливаю флаг на том нике за которого в данный момент играет чел
							Q_snprintf(buffer, sizeof(buffer), "UPDATE players SET current=%d WHERE user_steam_id='%s' AND user_name='%s'", 1, par->Player[i].cSteamId, par->Player[i].cNick);
							if (mysql_query (&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); }
						}
						else // Если нету совпадений, то создаю новый аккаунт (добавляю новый ИД в базу)
						{
							Q_snprintf(buffer, sizeof(buffer), "UPDATE players SET current=%d WHERE user_steam_id='%s'", 0, par->Player[i].cSteamId);
							if (mysql_query (&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection");}
							Q_snprintf(buffer, sizeof(buffer), "INSERT INTO players (user_steam_id, user_name, current) VALUES ('%s', '%s', %d)", par->Player[i].cSteamId, par->Player[i].cNick, 1);
							if (mysql_query (&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); }
						}
					}
					mysql_free_result(myBase->res); // Освобождаем память, занятую результирующей таблицей (всегда после mystat->res = mysql_store_result(&mystat->conn))
				}
				
				//////////////////////////////////
				//базовые ачивки
				// получаю весь список ачивок у игрока
				///////////////////////
				//
				//mysql_query Выполняет SQL запрос указанный query к базе данных, указанной в mysql.
				//Эта функция возвращает 0, если запрос выполнен успешно.
				//Результат, отличный от нуля, указывает на ошибку.
				//Обращение к mysql_error выведет текстовое сообщение об ошибке. 
				//Вызов mysql_num_rows даст Вам число строк, возвращенных запросом.
				//
				//пробелы впереди запроса не допустимы, иначе ошибка сразу в запросе. вот пипец - долго соображал что за косяк был.
				//и обязательно следить за правами на таблицу по доступу
				/////////////
				
				int tPoints = 0; // кол-во очков полученное за миссию (на игрока)
				int rows = 0;

				int teamkiller = 0;
				if (par->Player[i].iKiller >= 3) {
					teamkiller = 1;
					tPoints =- 14; //отнимаем 14 очков от общего количества очков
				}

				//записываю основную статистику
				Q_snprintf(buffer, sizeof(buffer), "INSERT INTO base_stat (\
					user_steam_id, \
					difficulty, \
					points, \
					time_try, \
					...
					Suicide, \
					c_IP\
					) VALUES ('%s', '%s', %d, '%s', '%s', '%s', %d, %d, %i, %.2f, %.2f, %d, %d, %d, %d, %d, %d, '%s', %d, %d, %d, %d, '%s')",
					par->Player[i].cSteamId, // user_steam_id
					par->cSkill, // difficulty
					tPoints, // очки за миссию;
					par->cMissionEndTime, // время вида yyyymmddhhmmss для MySQL timestamp (время окончания попытки)
					par->mapId, // map id
					par->campaign_id, //camaign id (from maps, not from base)
					(par->bMissionSuccess) ? 1 : 0, // wins stat1
					(par->bMissionSuccess) ? 0 : 1, // losses stat2
					(par->Player[i].bAlive) ? 0 : 1, // 0 - живой, 1 - помер. Короче записываю смерти
					par->Player[i].fFriendlyFireDamageDealt,
					par->Player[i].fDamageTaken,
					par->Player[i].iAliensKilled,
					par->Player[i].iMineKills,
					par->Player[i].iShieldbugsKilled,
					par->Player[i].iParasitesKilled,
					par->Player[i].iSentryKills,
					par->Player[i].iEggKills,
					par->Player[i].cProfile,
					par->iMissionTime,
					par->Player[i].iKiller,
					teamkiller, //Тимкиллер если 4 или более завалил
					par->Player[i].iSuicide,
					par->Player[i].c_IP);

				if (mysql_query (&myBase->conn, buffer) != 0) { bStatError = true; myMessage("Error: problem with connection!"); }

				if (par->ach_count >= 0) { //есть ачивки для подчета на данной сложности, иначе ваще ни че не считаю
					ach_player achplayer[200];
					int ach_player_count = -1;
					Q_snprintf(buffer, sizeof(buffer), "SELECT ach_id, difficulty, map_id, requirement FROM ach_stat WHERE user_steam_id='%s' AND (difficulty='%s' OR difficulty='*') AND (map_id='%s' OR map_id='*')",  par->Player[i].cSteamId, par->cSkill, par->mapId);
					if(mysql_query(&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
						myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
						if (&myBase->res != NULL) {
							rows = mysql_num_rows(myBase->res);
							if(rows > 0) { // Если имеется хотя бы одна запись (если есть какие-то ачивки), то сливаю данные в массив
								while((myBase->row = mysql_fetch_row(myBase->res)) != NULL) {//сливаю в массив
									ach_player_count++;
									achplayer[ach_player_count].id = atoi (myBase->row[0]); //ach_id;
									//row[0] - это первое поле, row[1] - второе поле
									// DIFF: 1-Easy; 2-Normal; 3-Hard; 4-Insane; 5-Brutal
									if (strcmp("*", myBase->row[1]) == 0) achplayer[ach_player_count].diff = 0; else
									if (strcmp("Easy", myBase->row[1]) == 0) achplayer[ach_player_count].diff = 1; else
									if (strcmp("Normal", myBase->row[1]) == 0) achplayer[ach_player_count].diff = 2; else
									if (strcmp("Hard", myBase->row[1]) == 0) achplayer[ach_player_count].diff = 3; else
									if (strcmp("Insane", myBase->row[1]) == 0) achplayer[ach_player_count].diff = 4; else
									if (strcmp("Brutal", myBase->row[1]) == 0) achplayer[ach_player_count].diff = 5;
									strcpy(achplayer[ach_player_count].mapId, myBase->row[2]); //map_id  (achplayer[ach_player_count].map = map_id)
									achplayer[ach_player_count].requirement = atoi (myBase->row[3]); //requirement
							...
							//======================
							//работаю с ачивками (общий смысл такой: иду по всему списку известных ачивок и далее смотрю какие есть у игрока и далее принимаю решения
							//======================
							//Msg ("========= TOTAL %s ACHIVES %d\n", pInf->GetPlayerName(), ach_player_count);
							for ( int ii = 0; ii <= par->ach_count; ii++ ) {// цикл по всем известным описанным ачивкам (ach_list[ii])
								if (ach_player_count >= 0) {
									for ( int jj = 0; jj <= ach_player_count; jj++ ) {//иду по ачивкам игрока и помечаю что они уже есть
										//Msg ("ID - %d: DIFF - %d\n", achplayer[jj].id, achplayer[jj].diff);
										//если есть ачивка (учитывается сложность и карта)
										//Msg ("IN::ACHid = %d; ach_list[%d].AchAlreadyInBase=%d\n", ach_list[ii].id, ii, ach_list[ii].AchAlreadyInBase);
										if (par->ach_list[ii].id == achplayer[jj].id && par->ach_list[ii].diff == achplayer[jj].diff && (strcmp(achplayer[jj].mapId, par->mapId) == 0 || strcmp(achplayer[jj].mapId, "*") == 0)) {
											par->ach_list[ii].newline = false; //true - insert; false - update; default = true
											par->ach_list[ii].current_req = achplayer[jj].requirement; //сколько уже есть в базе
											if (achplayer[jj].requirement >= par->ach_list[ii].requirement)
												par->ach_list[ii].AchAlreadyInBase = true;
											break;
										}
									}
								}
								//Msg ("TOTAL %d: Nick - %s: ach_list[%d].AchAlreadyInBase - %d    \n", ach_list[ii].id, pInf->GetPlayerName(), ii, ach_list[ii].AchAlreadyInBase);
								bool HaveAch = true;
								if (strcmp(ach_list[ii].mapId, "*") == 0 && !par->ach_list[ii].AchAlreadyInBase) HaveAch = false; // любая карта
								else if (!par->ach_list[ii].AchAlreadyInBase && strcmp(par->mapId, par->ach_list[ii].mapId) == 0 ) HaveAch = false; //если просто ачивки нету и карта совпадает из ачивки с нашей текущей картой
								if (!HaveAch) {
									//Msg ("NO ACH ID %d: Nick - %s: ach_list[%d].AchAlreadyInBase - %d    \n", ach_list[ii].id, pInf->GetPlayerName(), ii, ach_list[ii].AchAlreadyInBase);
									switch (par->ach_list[ii].id) {

										case 1 : // ID 1 - Mission Complete
											if (par->bMissionSuccess) { // условие для выполнения ачивки
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
											}
											break;
										
										case 2 : // ID 2 - No Flamer
											if (par->bMissionSuccess && !par->bFlamerUsed) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
											}
											break;
										
										case 3 : // ID 3 - Sound Execution
											if (par->bMissionSuccess && par->bAllMarinesAlive) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
											}
											break;
										
										case 4 : // ID 4 - No Flamer Sound Execution
											if (par->bMissionSuccess && !par->bFlamerUsed && par->bAllMarinesAlive) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;
										
										case 5 : // ID 5 - Speedrun
											if (par->bMissionSuccess && par->iMissionTime <= par->iSpeedrunTime) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
											}
											break;
										
										case 6 : // ID 6 - No Flamer Speedrun
											if (par->bMissionSuccess && !par->bFlamerUsed && par->iMissionTime <= par->iSpeedrunTime) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;
										
										case 7 : // ID 7 - Speedrun Sound Execution
											if (par->bMissionSuccess && par->iMissionTime <= par->iSpeedrunTime && par->bAllMarinesAlive) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;
										
										case 8 : // ID 8 - No Flamer Speedrun Sound Execution
											if (par->bMissionSuccess && !par->bFlamerUsed && par->iMissionTime <= par->iSpeedrunTime && par->bAllMarinesAlive) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;
										
										case 9 : // ID 9 - No Adren
											if (par->bMissionSuccess && par->iAdrenUsed <= 0) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;

										case 10 : // ID 10 - No Flamer No Adren
											if (par->bMissionSuccess && par->iAdrenUsed <= 0 && !par->bFlamerUsed) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;

										case 11 : // ID 11 - No Adren Sound Execution
											if (par->bMissionSuccess && par->iAdrenUsed <= 0 && par->bAllMarinesAlive) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;
											
										case 12 : // ID 12 - No Adren No Flamer Sound Execution
											if (par->bMissionSuccess && par->iAdrenUsed <= 0 && !par->bFlamerUsed && par->bAllMarinesAlive) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;

										case 13 : // ID 13 - Sector Destruction 400
											if (par->bMissionSuccess && par->iTotalKills >= 400) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;

										case 14 : // ID 14 - Sound Execution Sector Destruction 400
											if (par->bMissionSuccess && par->iTotalKills >= 400 && par->bAllMarinesAlive) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получил
											}
											break;

										case 15 : // ID 15 - Complexity (даем за просто прохождение сложности, например проешл Hard.
											//в базу эту ачивку не записываем, т.к. она дается постоянно
											if (par->bMissionSuccess == 1) {
												tPoints += par->ach_list[ii].points;
											}
											break;

										case 17 : // ID 17 - Classic Team
											/*
											прохождения  миссий при наличии в команде представителей всех классов - дается за каждую карту
											Все классы это - как минимум 1 офицер, 1 оружейник, 1 медик и 1 техник.
											(даем за просто прохождение сложности, например проешл Hard.
											*/
											if (par->bMissionSuccess && par->bAllMarinesAlive && par->bHasAllClasses) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;

										case 18 : // ID 18 - Perfect
											/*
											прохождение миссии без получения урона
											дается разово для каждой сложности (карта не учитывается)
											*/
											//if (par->bMissionSuccess && par->Player[i].bPerfect) {
											if (par->bMissionSuccess && par->Player[i].bPerfect) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;

										case 19 : // ID 19 - Perfect Team
											/*
											perfect  у всей команды одновременно
											разово для каждой сложности (карта не учитывается)
											*/
											if (par->bMissionSuccess && par->bPerfectTeam) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;

										case 20 : // ID 20 - Horde killer
											/*
											за текущую миссию убить 200 мобов
											(дается разово)
											*/
											//if (par->bMissionSuccess && par->Player[i].bPerfect) {
											if (par->bMissionSuccess && par->Player[i].iAliensKilled >= 200) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;

										case 21 : // ID 21 - Panzerschreck
											/*
											Убить 3 танка за одну карту(рассчитывается индивидуально, дается разово)
											*/
											//if (par->bMissionSuccess && par->Player[i].bPerfect) {
											if (par->bMissionSuccess && par->Player[i].iShieldbugsKilled >= 3) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;
										
										case 22 : // ID 22 - Jacob's No Tesla
											if (par->bMissionSuccess && !par->bTeslaGunUsed) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;

										case 23 : // ID 23 - Awesome Team
											/*
											каждого перса из комнады побили не более чем на 40% дается разово на
											харде инсе и бруте для всей команды
											(SE)
											*/
											if (par->bMissionSuccess && par->bAllMarinesAlive && par->bAwesome) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;

										case 24 : // ID 24 - Kill Them All
											if (par->bMissionSuccess && par->iSkill >= 3) { //от харда и выше
												int req = par->ach_list[ii].current_req + par->Player[i].iAliensKilled; //!!!!!!!!!!!!!!
												int achPoints = 0;
												bool show;
												if (req >= par->ach_list[ii].requirement) {
													achPoints = par->ach_list[ii].points;
													req = par->ach_list[ii].requirement;
													show = true; //показываем что дали ачивку и даём за неё очки
												} else
													show = false;
												if (par->ach_list[ii].newline) { //insert new line (insert ach)
													ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, ach_list[ii].diffS, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, achPoints, par->cMissionEndTime, par->Player[i].cNick, req, false, show);
												} else { //find and update current line (update ach)
													ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, ach_list[ii].diffS, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, achPoints, par->cMissionEndTime, par->Player[i].cNick, req, true, show);
												}
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
												par->ach_list[ii].newline = false;
											}
											break;

										case 25 : // ID 25 - Nuke from orbit //от харда и выше
											if (par->bMissionSuccess && par->iSkill >= 3) {
												int req = par->ach_list[ii].current_req + par->Player[i].iAliensKilled; //!!!!!!!!!!!!!!
												int achPoints = 0;
												bool show;
												if (req >= par->ach_list[ii].requirement) {
										...
										case 26 : // ID 26 - Le Docteur
											if (par->bMissionSuccess && par->bAllMarinesAlive && (strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0 || strcmp("#asw_name_faith", par->Player[i].cProfile) == 0)) {
												//Msg("********MAP:%s\n", par->ach_list[ii].mapId);
												//Msg("********DIFF_from base:%s\n", par->ach_list[ii].diffS);
												//Msg("********DIFF_Current:%s\n", par->cSkill);
												if (strcmp("*", par->ach_list[ii].mapId) != 0) {
													//&& strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0)
													if (strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0 && strcmp("ASI-Jac4-Residential", par->mapId) == 0 && !par->bFlamerUsed) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0 && strcmp("as_sci2_sewer", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_faith", par->Player[i].cProfile) == 0 && strcmp("dc1-omega_city", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													}
												}
												if (strcmp("*", par->ach_list[ii].mapId) == 0) { //делаю запрос на получение всех нужных мне ачивок-условий для главной ачивки
													Q_snprintf(buffer, sizeof(buffer), "SELECT * FROM ach_stat WHERE ach_id='%d' AND user_steam_id='%s' AND difficulty='%s' AND map_id IN ('ASI-Jac4-Residential', 'as_sci2_sewer', 'dc1-omega_city')",  par->ach_list[ii].id, par->Player[i].cSteamId, par->ach_list[ii].diffS);
													if (mysql_query(&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
														myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
														if (&myBase->res != NULL) {
															rows = mysql_num_rows(myBase->res);
															if (rows == 3) { // Если имеем все три карты (записи) то записываю главную ачивку.
																ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
																par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
															}
														}
										...
										case 27 : // ID 27 - Pan Oficer
											if (par->bMissionSuccess && par->bAllMarinesAlive && (strcmp("#asw_name_jaeger", par->Player[i].cProfile) == 0 || strcmp("#asw_name_sarge", par->Player[i].cProfile) == 0)) {
												if (strcmp("*", par->ach_list[ii].mapId) != 0) {
													//&& strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0)
													if (strcmp("#asw_name_jaeger", par->Player[i].cProfile) == 0 && strcmp("as_paranoia2", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_jaeger", par->Player[i].cProfile) == 0 && strcmp("as_sci3_maintenance", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_sarge", par->Player[i].cProfile) == 0 && strcmp("TFT-Spaceport", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													}
												}
												if (strcmp("*", par->ach_list[ii].mapId) == 0) { //делаю запрос на получение всех нужных мне ачивок-условий для главной ачивки
													Q_snprintf(buffer, sizeof(buffer), "SELECT * FROM ach_stat WHERE ach_id='%d' AND user_steam_id='%s' AND difficulty='%s' AND map_id IN ('as_paranoia2', 'as_sci3_maintenance', 'TFT-Spaceport')",  par->ach_list[ii].id, par->Player[i].cSteamId, par->ach_list[ii].diffS);
													if (mysql_query(&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
														myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
														if (&myBase->res != NULL) {
															rows = mysql_num_rows(myBase->res);
										...
										case 28 : // ID 28 - Der Techniker
											if (par->bMissionSuccess && par->bAllMarinesAlive && (strcmp("#asw_name_vegas", par->Player[i].cProfile) == 0 || strcmp("#asw_name_crash", par->Player[i].cProfile) == 0)) {
												if (strcmp("*", par->ach_list[ii].mapId) != 0) {
													//&& strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0)
													if (strcmp("#asw_name_vegas", par->Player[i].cProfile) == 0 && strcmp("ASI-Jac7-TimorStation", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_crash", par->Player[i].cProfile) == 0 && strcmp("as_sci3_maintenance", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_vegas", par->Player[i].cProfile) == 0 && strcmp("as_paranoia2", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													}
												}
												if (strcmp("*", par->ach_list[ii].mapId) == 0) { //делаю запрос на получение всех нужных мне ачивок-условий для главной ачивки
													Q_snprintf(buffer, sizeof(buffer), "SELECT * FROM ach_stat WHERE ach_id='%d' AND user_steam_id='%s' AND difficulty='%s' AND map_id IN ('ASI-Jac7-TimorStation', 'as_sci3_maintenance', 'as_paranoia2')",  par->ach_list[ii].id, par->Player[i].cSteamId, par->ach_list[ii].diffS);
													if (mysql_query(&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
														myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
														if (&myBase->res != NULL) {
															rows = mysql_num_rows(myBase->res);
										...
										case 29 : // ID 29 - El Luchador
											if (par->bMissionSuccess && par->bAllMarinesAlive && (strcmp("#asw_name_wolfe", par->Player[i].cProfile) == 0 || strcmp("#asw_name_wildcat", par->Player[i].cProfile) == 0)) {
												if (strcmp("*", par->ach_list[ii].mapId) != 0) {
													//&& strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0)
													if (strcmp("#asw_name_wolfe", par->Player[i].cProfile) == 0 && strcmp("as_paranoia2", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_wildcat", par->Player[i].cProfile) == 0 && strcmp("ASI-Jac4-Residential", par->mapId) == 0 && !par->bFlamerUsed) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													} else if (strcmp("#asw_name_wolfe", par->Player[i].cProfile) == 0 && strcmp("as_sci1_bridge", par->mapId) == 0) {
														ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, par->mapId, par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick, 1, false, false);
														par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивка получена
													}
												}
												if (strcmp("*", par->ach_list[ii].mapId) == 0) { //делаю запрос на получение всех нужных мне ачивок-условий для главной ачивки
													Q_snprintf(buffer, sizeof(buffer), "SELECT * FROM ach_stat WHERE ach_id='%d' AND user_steam_id='%s' AND difficulty='%s' AND map_id IN ('as_paranoia2', 'ASI-Jac4-Residential', 'as_sci1_bridge')",  par->ach_list[ii].id, par->Player[i].cSteamId, par->ach_list[ii].diffS);
													if (mysql_query(&myBase->conn, buffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
														myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
														if (&myBase->res != NULL) {
															rows = mysql_num_rows(myBase->res);
										...
										case 30 : // ID 30 - Rambo  убить 100 мобов по 50 раз = 5000 мобов всего убить
											if (par->bMissionSuccess && par->Player[i].iAliensKilled >= 100 && (strcmp("#asw_name_wolfe", par->Player[i].cProfile) == 0 || strcmp("#asw_name_wildcat", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int cond = 100; //!!!!!!!!!!!!!!!!! убить 100 мобов по 50 раз = 5000 мобов всего убить
												int achPoints = 0;
												int req = par->ach_list[ii].current_req + cond;
												bool show;
												if (req >= par->ach_list[ii].requirement) {
													achPoints = par->ach_list[ii].points;
													req = par->ach_list[ii].requirement;
										...
										case 31 : // ID 31 - Firewall Specialist  убить 100 мобов по 50 раз = 5000 мобов всего убить
											if (par->bMissionSuccess && par->Player[i].iMineKills >= 100 && (strcmp("#asw_name_jaeger", par->Player[i].cProfile) == 0 || strcmp("#asw_name_sarge", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int cond = 100; //!!!!!!!!!!!!!!!!! убить 100 мобов по 50 раз = 5000 мобов всего убить
												int achPoints = 0;
												int req = par->ach_list[ii].current_req + cond;
												bool show;
										...
										case 32 : // ID 32 - Sniper  5 побед без ФФ. (за всё время игры)
											if (par->bMissionSuccess && par->Player[i].fFriendlyFireDamageDealt <= 0 && (strcmp("#asw_name_jaeger", par->Player[i].cProfile) == 0 || strcmp("#asw_name_sarge", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int cond = 1; //!!!!!!!!!!!!!!!!!
												int achPoints = 0;
												int req = par->ach_list[ii].current_req + cond;
												bool show;
										...
										case 33 : // ID 33 - Sharpshooter 5 побед без ФФ. (за всё время игры)
											if (par->bMissionSuccess && par->Player[i].fFriendlyFireDamageDealt <= 0 && (strcmp("#asw_name_wolfe", par->Player[i].cProfile) == 0 || strcmp("#asw_name_wildcat", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int cond = 1; //!!!!!!!!!!!!!!!!!
												int achPoints = 0;
												int req = par->ach_list[ii].current_req + cond;
												bool show;
										...
										case 34 : // ID 34 - ParaMedic спасение 3 заражения за миссию
											if (par->bMissionSuccess && par->Player[i].iCuredInfestation >= 3 && (strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0 || strcmp("#asw_name_faith", par->Player[i].cProfile) == 0)) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;

										case 35 : // ID 35 - iBolit выхилить 300 пунктов за миссию  50 раз
											if (par->bMissionSuccess && par->Player[i].iMedicHealing >= 300 && (strcmp("#asw_name_bastille", par->Player[i].cProfile) == 0 || strcmp("#asw_name_faith", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int cond = 300; //!!!!!!!!!!!!!!!!!
												int achPoints = 0;
												int req = par->ach_list[ii].current_req + cond;
												bool show;
										...
										case 36 : // ID 36 - smart hacker   50  бездреновых фастхаков (змейка без дрена)
											if (par->bMissionSuccess && par->Player[i].iNoAdrenFastDoorHacks >= 1 && (strcmp("#asw_name_vegas", par->Player[i].cProfile) == 0 || strcmp("#asw_name_crash", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int req = par->ach_list[ii].current_req + par->Player[i].iNoAdrenFastDoorHacks; //!!!!!!!!!!!!!!
												int achPoints = 0;
												bool show;
												if (req >= par->ach_list[ii].requirement) {
													achPoints = par->ach_list[ii].points;
										...
										case 37 : // ID 37 - Circuit Breaker   50  фастхаков (змейка)
											if (par->bMissionSuccess && par->iSkill >= 3 && par->Player[i].iFastDoorHacks >= 1 && (strcmp("#asw_name_vegas", par->Player[i].cProfile) == 0 || strcmp("#asw_name_crash", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int req = par->ach_list[ii].current_req + par->Player[i].iFastDoorHacks; //!!!!!!!!!!!!!!
												int achPoints = 0;
												bool show;
												if (req >= par->ach_list[ii].requirement) {
													achPoints = par->ach_list[ii].points;
										...
										case 38 : // ID 38 - Security Expert   50  фастхаков компьютеров
											if (par->bMissionSuccess && par->iSkill >= 3 && par->Player[i].iFastComputerHacks >= 1 && (strcmp("#asw_name_vegas", par->Player[i].cProfile) == 0 || strcmp("#asw_name_crash", par->Player[i].cProfile) == 0)) { //!!!!!!!!!!!!!!
												int req = par->ach_list[ii].current_req + par->Player[i].iFastComputerHacks; //!!!!!!!!!!!!!!
												int achPoints = 0;
												bool show;
												if (req >= par->ach_list[ii].requirement) {
													achPoints = par->ach_list[ii].points;
										...
										case 39 : // ID 39 - Para Chopper   4 паразита пилой (только техниками)
											if (par->bMissionSuccess && par->Player[i].iParasitesKilledByChainsaw >= 4 && (strcmp("#asw_name_vegas", par->Player[i].cProfile) == 0 || strcmp("#asw_name_crash", par->Player[i].cProfile) == 0)) {
												ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, par->ach_list[ii].id, par->ach_list[ii].ach_name, par->ach_list[ii].points, par->cMissionEndTime, par->Player[i].cNick);
												par->ach_list[ii].AchAlreadyInBase = true; //сообщаем массиву что ачивку получил
											}
											break;
								...
								//сбрасываю флаг ачивки (если она стала = true), т.к. общий массив ачивок считывается всего один раз. и поэтому для следующих игроков будем считать что ачивок нету
								par->ach_list[ii].AchAlreadyInBase = false;
								par->ach_list[ii].newline = true;
								par->ach_list[ii].current_req = 0;
							} // закрыл цикл по всем ачивкам (конец работы с ачивками)
						}
						else {
							bStatError = true; 
						}
						mysql_free_result(myBase->res); // обязательно освобождаем память, занятую результирующей таблицей (всегда после mystat->res = mysql_store_result)
					}
				}
				
				//Дописываю основную статистику
				//нужно сделать UPDATE!!! (для очков)
				Q_snprintf(buffer, sizeof(buffer), "UPDATE base_stat SET points='%d' WHERE user_steam_id='%s' AND difficulty='%s' AND map_id='%s' AND time_try='%s' AND campaign_id='%s' AND c_IP='%s' AND profile='%s'",
					tPoints, par->Player[i].cSteamId, par->cSkill, par->mapId, par->cMissionEndTime, par->campaign_id, par->Player[i].c_IP, par->Player[i].cProfile);
				if (mysql_query (&myBase->conn, buffer) != 0) { bStatError = true; myMessage("Error: problem with connection!"); }
				
				char vbuffer[512];
				// ID 40 - VETERAN
				if (par->bMissionSuccess && par->missions_num != 0 && par->Player[i].bAlive && par->iSkill >= 3) {

					bool ach40list = false;
					char ach40name[50] = "Veteran";
					int ach40points = 1;

					Q_snprintf(vbuffer, sizeof(vbuffer), "SELECT ach_name, ach_points FROM ach_list WHERE ach_difficulty='%s' AND ach_id='40'", par->cSkill);
					if(mysql_query(&myBase->conn, vbuffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
						myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
						if (&myBase->res != NULL) {
							if(mysql_num_rows(myBase->res) > 0) { // Если имеется хотя бы одна запись
								ach40list = true;
								while((myBase->row = mysql_fetch_row(myBase->res)) != NULL) {//сливаю в массив
									ach40points = atoi (myBase->row[1]);
									strcpy(ach40name, myBase->row[0]);
								}	
							}
						}
						mysql_free_result(myBase->res); // Освобождаем память, занятую результирующей таблицей (всегда после mystat->res = mysql_store_result(&mystat->conn))
					...
					if (ach40list) {
						...
						// проверка: если есть уже ачивка на такой сложности и на такой кампании
						bool ach40 = true;
						Q_snprintf(vbuffer, sizeof(vbuffer), "SELECT * FROM ach_stat WHERE user_steam_id='%s' AND campaign_id='%s' AND difficulty='%s' AND ach_id='40'", par->Player[i].cSteamId, par->campaign_id, par->cSkill);
						if(mysql_query(&myBase->conn, vbuffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
							myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
							if (&myBase->res != NULL) {
								if(mysql_num_rows(myBase->res) > 0) { // Если имеется хотя бы одна запись
						...
						if (ach40) {
							Q_snprintf(vbuffer, sizeof(vbuffer), "SELECT map_id FROM base_stat WHERE user_steam_id='%s' AND campaign_id='%s' AND difficulty='%s' AND wins='1' GROUP BY map_id", par->Player[i].cSteamId, par->campaign_id, par->cSkill);
							if(mysql_query(&myBase->conn, vbuffer) != 0) { bStatError = true; myMessage("problem with connection"); } else {
								myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
								if (&myBase->res != NULL) {
									int mNums = mysql_num_rows(myBase->res);
									// debug
									if (mNums == par->missions_num) { // Если количество карт равно количеству миссий в компании
										//save achievements
										ACH_SaveToBase (myBase, par->mapName, par->Player[i].cSteamId, par->cSkill, "*", par->campaign, par->campaign_id, 40, ach40name, ach40points * par->missions_num, par->cMissionEndTime, par->Player[i].cNick);
									}
								}
								mysql_free_result(myBase->res); // Освобождаем память, занятую результирующей таблицей (всегда после mystat->res = mysql_store_result(&mystat->conn))
							}
	...
	} else myMessage(NOSTAT);
	
	return 0;
}

//void GetBaseData(void* params)
//unsigned GetBaseData(void* params)
unsigned __stdcall GetBaseData(void* params)
{
	...
	//tData* par = (tData*) params; // always use a struct!
	tBaseData* par = (tBaseData*) params; // always use a struct!
	cSQL* myBase = new cSQL;

	strcpy(CurrentMap.mapId, par->mapId); // id
	strcpy(CurrentMap.name, par->mapId); // name
	strcpy(CurrentMap.campaign, "*"); // name
	
	CASW_Campaign_Info *pCampaign = ASWGameRules()->GetCampaignInfo();

	if ( pCampaign ) {
		//Msg("************** GOOOOOOOOOOOOOOOOOOOOOOODDDDDDD\n");
		CurrentMap.missions_num = pCampaign->GetNumMissions()-1;
		strcpy(CurrentMap.campaign_id, pCampaign->m_szCampaignFilename);
		//Msg("*** NUMS: %d\n", CurrentMap.missions_num);
		//Msg("*** ID: %s\n", CurrentMap.campaign_id);
	} else {
		//Msg("************** FAIIIIIIIIIIIIIILLLLLLLLLLLL\n");
		CurrentMap.missions_num = 0;
		strcpy(CurrentMap.campaign_id, "*");
	}

	CurrentMap.speedrun = 0; //time
	strcpy(CurrentMap.cSkill, par->cSkill); // уровень сложности "текст"
	CurrentMap.iSkill = par->iSkill; // уровень сложности "целое"
	...
	if ( !myBase->cInit() ) { myMessage(NOSTAT); bStatError = true; } else {
		...
		// получение спидрана и имени карты из базы
		// данные записывает в глобальную переменную CurrentMap (структура: id, name, speedrun)
		Q_snprintf(buffer, sizeof(buffer), "SELECT speedrun, map_name, campaign FROM maps WHERE map_id='%s'", STRING(gpGlobals->mapname)); //id карты для поиска
		if(mysql_query(&myBase->conn, buffer) != 0) { myMessage("Error calculating speedrun!"); bStatError = true; } else {
			myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
			if (&myBase->res != NULL) {
				if(mysql_num_rows(myBase->res) > 0) { // Если имеется хотя бы одна запись
					...
					myBase->row = mysql_fetch_row(myBase->res);
					if (myBase->row != NULL) {
						if (myBase->row[0] != NULL)
							CurrentMap.speedrun = atoi (myBase->row[0]);
						if (myBase->row[1] != NULL)
							strcpy(CurrentMap.name, myBase->row[1]); //полное имя карты на выходе, не id
						if (myBase->row[2] != NULL)
							strcpy(CurrentMap.campaign, myBase->row[2]); //полное наименование компании на выходе
					}
		...
		//Загружаю таблицу (список) ачивок, по которым буду в дальшейшем ходить
		//данные записывает в глобальный массив ach_list[200]; //id, diff, points
		int ach_count = -1;
		CurrentMap.ach_count = ach_count;

		Q_snprintf(buffer, sizeof(buffer), "SELECT ach_id, ach_difficulty, ach_points, ach_name, mapId, requirement FROM ach_list WHERE (ach_difficulty='%s' OR ach_difficulty='*')", par->cSkill);
		if(mysql_query(&myBase->conn, buffer) != 0) { myMessage("Error getting ach list!"); bStatError = true; } else {
			myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
			if (&myBase->res != NULL) {
				if(mysql_num_rows(myBase->res) > 0) { // Если имеется хотя бы одна запись
					while((myBase->row = mysql_fetch_row(myBase->res)) != NULL) { //записываю всё в массив
						ach_count++;
						//row[0] - это первое поле (ach_id), row[1] - второе поле (ach_difficulty)
						ach_list[ach_count].id = atoi (myBase->row[0]); //ach_id
							
						// DIFF: 1-Easy; 2-Normal; 3-Hard; 4-Insane; 5-Brutal
						if (strcmp("*", myBase->row[1]) == 0) ach_list[ach_count].diff = 0; else
						if (strcmp("Easy", myBase->row[1]) == 0) ach_list[ach_count].diff = 1; else
						if (strcmp("Normal", myBase->row[1]) == 0) ach_list[ach_count].diff = 2; else
						if (strcmp("Hard", myBase->row[1]) == 0) ach_list[ach_count].diff = 3; else
						if (strcmp("Insane", myBase->row[1]) == 0) ach_list[ach_count].diff = 4; else
						if (strcmp("Brutal", myBase->row[1]) == 0) ach_list[ach_count].diff = 5;
						strcpy(ach_list[ach_count].diffS, myBase->row[1]); //diff string

						ach_list[ach_count].points = atoi (myBase->row[2]); //ach_points
						strcpy(ach_list[ach_count].ach_name, myBase->row[3]); //ach_name  (ach_list[ach_count].name = ach_name)

						strcpy(ach_list[ach_count].mapId, myBase->row[4]); //mapId

						ach_list[ach_count].requirement = atoi (myBase->row[5]); //requirement
							
						ach_list[ach_count].AchAlreadyInBase = false;
						ach_list[ach_count].newline = true;
						//ach_list[ach_count].current_req = ach_list[ach_count].requirement;
						ach_list[ach_count].current_req = 0;
					}
					CurrentMap.ach_count = ach_count; //записываем количество ачивок для текущей миссии
				}
			}
			mysql_free_result(myBase->res); // Освобождаем память, занятую результирующей таблицей (всегда после myBase->res = mysql_store_result(&myBase->conn))
		}
...
#define ASW_LAUNCHING_STEP 0.25f			// time between each stage of launching
	...
	ConVar asw_adjust_difficulty_by_number_of_marines( "asw_adjust_difficulty_by_number_of_marines", "1", FCVAR_NONE, "If enabled, difficulty will be reduced when there are only 3 or 2 marines." );
...
ConVar mm_max_players( "mm_max_players", "4", FCVAR_REPLICATED, "Max players for matchmaking system" );
...
CAlienSwarm::CAlienSwarm()
{
	...
	//zak выставляю нужные настройки сервера
	extern ConVar asw_realistic_death_chatter; asw_realistic_death_chatter.SetValue(1);
	engine->ServerCommand("mp_friendlyfire 1\n");
	asw_sentry_friendly_fire_scale.SetValue( 1 );
	asw_marine_ff_absorption.SetValue( 0 );
	extern ConVar asw_marine_ff; asw_marine_ff.SetValue(2);
	extern ConVar asw_marine_ff_dmg_base; asw_marine_ff_dmg_base.SetValue(3);
	extern ConVar asw_custom_skill_points; asw_custom_skill_points.SetValue(0);
	asw_adjust_difficulty_by_number_of_marines.SetValue(0);
	extern ConVar asw_marine_death_protection; asw_marine_death_protection.SetValue(0);
	asw_marine_collision.SetValue(1);
	asw_horde_override.SetValue(1);
	asw_wanderer_override.SetValue(1);

	mm_max_players.SetValue(6);
	engine->ServerCommand("sv_visiblemaxplayers 4\n");

	myTextParam = new hudtextparms_s;

	//new version window
	/*
	if (bNewVersion)
		if (CurrentNewView)
			(HANDLE)_beginthreadex(NULL, 0, UpdateIt, 0, 0, NULL);
	*/
...
{	
	...
	if (ASWGameResource())
	{
		...
		if (index >= 0 && index < 8)
		{
			//Zak - ставлю автогалку готовности (автореди)
		...
		if (ASWGameResource()->GetLeader() == NULL)
		{
			...
			//Zak - повтор проставления галки, если вдруг сменился лидер позже
			if (index >= 0 && index < 8)
			{
				//Zak - ставлю автогалку готовности (автореди)
				//ASWGameResource()->m_bPlayerReady.Set(index, false);
		...
		//zak
		char ip[30], *ptr;
		strcpy(ip, pszAddress);
		if ((ptr = strchr(ip, ':')) != NULL)
			*ptr = '\0';
		strcpy (PlayerData[index+1].c_IP, ip); //запоминаем IP клиента	

		/*
		//после смены лидера проставляем всем галки готовности
		for (int i=0;i<ASW_MAX_READY_PLAYERS;i++) {
			ASWGameResource()->m_bPlayerReady.Set(i, true);
		}
		*/
	}
...
bool CAlienSwarm::RosterSelect( CASW_Player *pPlayer, int RosterIndex, int nPreferredSlot )
{
	...
	if (!ASWGameResource()->IsRosterSelected(RosterIndex))		// if not already selected
	{
		...
		{						
			...
			if ( !ASWGameResource()->AddMarineResource( m, pPlayer ? pPlayer : NULL, nPreferredSlot ))
			{
				UTIL_Remove( m );
				return false;
			}
...
// 	CASW_Spawner::s_iNormalDronesSpawned = 0;
	...
	//Zak - Чтение нужных мне даннных из базы
	// получаю спидпан(speedrun) карты и полное имя в глобальну переменную CurrentMap (id, name, speedrun)
	// список ачивок
	
	BaseData = new tBaseData;
	switch( GetSkillLevel() )
	{
		case 1: strcpy(BaseData->cSkill, "Easy"); break;
		case 2: strcpy(BaseData->cSkill, "Normal"); break;
		case 3: strcpy(BaseData->cSkill, "Hard"); break;
		case 4: strcpy(BaseData->cSkill, "Insane"); break;
		case 5: strcpy(BaseData->cSkill, "Brutal"); break;
	}
	BaseData->iSkill = GetSkillLevel();
	strcpy(BaseData->mapId, STRING(gpGlobals->mapname));
	(HANDLE)_beginthreadex(NULL, 0, &GetBaseData, BaseData, 0, NULL);
	//***
...
void CAlienSwarm::RestartMission( CASW_Player *pPlayer, bool bForce )
{
	...
	UTIL_ClientPrintAll(HUD_PRINTTALK, "Restarting Mission");
	...
	else	
	{
		...
		if (ASWGameResource())
		{
			for (int i=0;i<ASW_MAX_READY_PLAYERS;i++)
			{
				...
				//Zak думаю что это после завершения миссии нужно ставить галки. вот и ставлю автореди
				ASWGameResource()->m_bPlayerReady.Set(i, true);
...
void CAlienSwarm::StartStim( float duration, CBaseEntity *pSource )
{
	...
	endadren = m_flStimEndTime;
	m_flStimStartTime = gpGlobals->curtime;
	m_hStartStimPlayer = pSource;

	//zak adren плюсую все адрены в том числе сломо
	m_iAdrenUsed++;
}
...
void CAlienSwarm::PlayerThink( CBasePlayer *pPlayer )
{
	...
	CASW_Player *pMar = ToASW_Player(pPlayer);
	if (!pMar)
		return;
	
	
	if ( pMar->HasFullyJoined() ) {

		if ( !pMar->m_bSentJoinedPoints ) {
			if (gpGlobals->curtime >= m_fBriefingStartedTime+1) {
				pMar->m_bSentJoinedPoints = true;

				CReliableBroadcastRecipientFilter filter;
				filter.RemoveRecipient( pPlayer );		// notify everyone except the player changing the difficulty level

				char buffer[128];
				Q_snprintf(buffer, sizeof(buffer), "%s has %d pts.\n", pMar->GetPlayerName(), pMar->m_iPointsOnJoined );
				//UTIL_ClientPrintAll( HUD_PRINTTALK, buffer );
		...
		if ( !pMar->m_bAllreadyWelcomed ) {
			//gpGlobals->curtime;
			if (gpGlobals->curtime >= m_fBriefingStartedTime+2) {
				pMar->m_bAllreadyWelcomed = true;
				...
				/*
				int pPoints = 0;
				//делаю запрос для получения очков для вывода на экран
				cSQL* myBase = new cSQL;
				if ( !myBase->cInit() ) {
					//myMessage("Connaction error!");
				} else {
		
					//Msg("****: %s\n", pPlayer->GetNetworkIDString());
					Q_snprintf(buffer, sizeof(buffer), "SELECT sum(pts) AS pPoints FROM (SELECT sum(points) AS pts FROM base_stat ta WHERE ta.user_steam_id='%s' UNION SELECT sum(ach_points) AS pts FROM ach_stat ta WHERE ta.user_steam_id='%s') AS pPoint", pPlayer->GetNetworkIDString(), pPlayer->GetNetworkIDString()); //CASW_Player->GetASWNetworkID()
					if(mysql_query(&myBase->conn, buffer) != 0) {
						//myMessage("Error calculating player points");
					} else {
						myBase->res = mysql_store_result(&myBase->conn); // Получаем дескриптор результирующей таблицы (проще сливаем данные в переменную)
						if (&myBase->res != NULL) {
							if(mysql_num_rows(myBase->res) > 0) { // Если имеется хотя бы одна запись
				...
				*/
				...
				//ClientPrint(pPlayer, HUD_PRINTTALK, "Welcome to ASB2 MOD.\ntype !help in main chat for more details\nasb2.org - statistics server\n");
				Q_snprintf(buffer, sizeof(buffer), "Welcome to ASB2 MOD.\ntype !help in main chat for more details\nasb2.org - statistics server\nYou have: %d pts.\n", pMar->m_iPointsOnJoined);
		
				ClientPrint(pPlayer, HUD_PRINTTALK, buffer);

				if (bLockedMap)
					ClientPrint(pPlayer, HUD_PRINTTALK, "Unknown map!\n");
				if (bStatError)
					ClientPrint(pPlayer, HUD_PRINTTALK, NOSTAT);
				if (bNewVersion)
					myMessage("NEW VERSION DETECTED! PLEASE UPDATE THIS SERVER");
			}
		}
	}
	...
	case ASW_GS_INGAME:
		{
			...
			if (ASWGameResource()) {
				...
				for ( int pl = 0; pl < ASWGameResource()->GetMaxMarineResources(); pl++ )
				{
					
					
					CASW_Marine_Resource *pMR = ASWGameResource()->GetMarineResource( pl );
					if ( !pMR )
						continue;
					if (!pMR->IsInhabited())
						continue;
					CASW_Player* pInfo = pMR->GetCommander();
					if (!pInfo)
						continue;
					tkil += pMR->m_iAliensKilled;
				}

				gTime = gpGlobals->curtime - m_fMissionStartedTime;
				Q_snprintf(time_buffer, sizeof(time_buffer), "%02d:%02d", (gTime % 3600)/60, (gTime % 3600) % 60);

				for ( int ii = 0; ii < ASWGameResource()->GetMaxMarineResources(); ii++ )
				{
					CASW_Marine_Resource *pMR = ASWGameResource()->GetMarineResource( ii );
					...
					myTextParam->b2			= 0;
					myTextParam->a2			= 255;
					myTextParam->fadeinTime	= 0.1;
					myTextParam->fadeoutTime	= 0.0;
					myTextParam->holdTime		= 12.0;
					myTextParam->fxTime		= 0;

					short idx = pInfo->entindex();
					// ZAK TIMER
					if (PlayerData[idx].bStatusTimer) {
					
						myTextParam->x			= -1; //float 0 to 1    (-1 center)
						myTextParam->y			= 0; // float 0 to 1    (-1 center)
						myTextParam->channel		= 1; // 0 to everyone / default 1
						UTIL_HudMessage(pInfo, *myTextParam, time_buffer );
					}
			
					// timer centered
					if (PlayerData[idx].bStatusTimerCenter) {
					
						ClientPrint(pInfo, HUD_PRINTCENTER, time_buffer);
					}

					//my kills
					if (PlayerData[idx].bStatusMyKills) {
						
				
						myTextParam->x			= -1; //float 0 to 1    (-1 center)
						myTextParam->y			= 1; // float 0 to 1    (-1 center)
						myTextParam->channel		= 3; // 0 to everyone / default 1

					
						if ((Q_stricmp(pMR->GetProfile()->GetShortName(), "#asw_name_jaeger") == 0) || (Q_stricmp(pMR->GetProfile()->GetShortName(), "#asw_name_sarge") == 0))
							Q_snprintf(buf, sizeof(buf), "Shoted: %d (total: %d) fm: %d", pMR->m_iAliensKilled, tkil, pMR->m_iMineKills);
						else if ((Q_stricmp(pMR->GetProfile()->GetShortName(), "#asw_name_bastille") == 0) || (Q_stricmp(pMR->GetProfile()->GetShortName(), "#asw_name_faith") == 0))
							Q_snprintf(buf, sizeof(buf), "Healing: %d Cured: %d", pMR->m_iMedicHealing, pMR->m_iCuredInfestation);
						else
							Q_snprintf(buf, sizeof(buf), "Shoted: %d (total: %d)", pMR->m_iAliensKilled, tkil);
						UTIL_HudMessage( pInfo, *myTextParam, buf );
					}
				}
...
void CAlienSwarm::MissionComplete( bool bSuccess )
{
	...
	if ( pGameResource )
	{
		...
		for ( int i = 0; i < ASW_MAX_READY_PLAYERS; i++ )
		{
			...
			//Zak автореди для выхода из брифинга
	...
	else if ( pGameResource )
	{
		...
		for ( int i = 0; i < ASW_MAX_MARINE_RESOURCES; i++ )
		{
			...
			//zak : фламер/огнемет flamers hit/shots
			m_hDebriefStats->m_iFlamerShots.Set(i, pMR->m_iFlamerFired );
			m_hDebriefStats->m_iFlamerHit.Set(i, pMR->m_iFlamerFired - pMR->m_iShotsMissed );
	...
	if (bBotUsed) {
		bStatError = true;
		myMessage("BOT DETECTED!!!");
		Warning("BOT DETECTED!!!\n");
	}
	if (!bNewVersion) {
	if (!bStatError) { //если сервер лежит то ничего не пишем.
		if (m_bCheated.Get()) myMessage(NOSTAT); else { // Были использованы читы - ни чего не обрабатываем
			if ( pGameResource && !IsTutorialMap() ) {

				SaveData = new tSaveData;
				SaveData->iMissionTime = m_hDebriefStats->m_fTimeTaken.Get(); // время затраченное на миссию;
				SaveData->iSpeedrunTime = CurrentMap.speedrun;
				SaveData->bAllMarinesAlive = true; //флаг все марины живы
				SaveData->bHasAllClasses = false; //начальный флаг - не все классы были в игре
				SaveData->bPerfectTeam = true; //начальный флаг - Все прошли в перфект
				SaveData->bFlamerUsed = false; // для ачивки. использовался ли фламер на миссии true - ДА
				SaveData->bAwesome = true; // для ачивки Awesome Team. true - всей командой получено меньше 40% урона. false - хоть у одного больше чем 40% урона (дамага)
				SaveData->bTeslaGunUsed = false;

				SaveData->bMissionSuccess = m_bMissionSuccess;
				strcpy (SaveData->mapId, CurrentMap.mapId);
				strcpy (SaveData->mapName, CurrentMap.name);
				strcpy (SaveData->cSkill, CurrentMap.cSkill);

				strcpy (SaveData->campaign_id, CurrentMap.campaign_id);
				SaveData->missions_num = CurrentMap.missions_num;
				strcpy (SaveData->campaign, CurrentMap.campaign);

				SaveData->iSkill = CurrentMap.iSkill;
				SaveData->ach_count = CurrentMap.ach_count;
				SaveData->iAdrenUsed = m_iAdrenUsed; // Сколько было адренов использованно за миссию (сломо заложенное по сюжету игры не входит в этот количество (игровое сломо))
				SaveData->iTotalKills = m_hDebriefStats->m_iTotalKills.Get();
			
				//экспериментальное копирование. если что, то сделать обычным циклом.
				//memcpy(ach_list,SaveData->ach_list,sizeof ach_list); //не пробовал
				for ( int i = 0; i <= CurrentMap.ach_count; i++ ) // цикл по клиентам (маринам)
					SaveData->ach_list[i] = ach_list [i];
			
				// вычисляю сколько живых и сколько реальных игроков осталось
				SaveData->maxClients = 0;
				short CurPlayer = 0, TechClass = 0, MedicClass = 0, OffClass = 0, GunClass = 0;


				

				//for ( int i = 1; i <= gpGlobals->maxClients; i++ ) // цикл по клиентам (маринам)
				for ( int i = 0; i < ASWGameResource()->GetMaxMarineResources(); i++ )
				{
					
					/*
					CASW_Player* pInf = dynamic_cast< CASW_Player* >( UTIL_PlayerByIndex( i ) );
					if (!pInf)
						continue;
					if (!ASWGameResource())
						continue;
					...
					*/
					...
					/*
					CASW_Marine_Resource *pMR = ASWGameResource()->GetMarineResource();
					//CASW_Marine_Resource *pMR = pInf->GetMarine()->GetMarineResource();
					if (!pMR)
						continue;
					if (!pMR->IsInhabited()) //если бот то пропускаем - не считаем стату
						continue;
					*/
					CASW_Marine_Resource *pMR = ASWGameResource()->GetMarineResource( i );
					...
					if (!pMR->IsInhabited())
						continue;
					//CBasePlayer* pInfo = UTIL_PlayerByIndex( ii+1 );
					CASW_Player* pInf = pMR->GetCommander();
					if (!pInf)
						continue;
					...
					SaveData->maxClients++;
					CurPlayer = SaveData->maxClients;
					strcpy (SaveData->Player[CurPlayer].cSteamId, pInf->GetASWNetworkID());
					strcpy (SaveData->Player[CurPlayer].cNick, pInf->GetPlayerName());
					strcpy (SaveData->Player[CurPlayer].cProfile, pMR->GetProfile()->GetShortName());
					strcpy (SaveData->Player[CurPlayer].c_IP, PlayerData[pInf->entindex()].c_IP);

					SaveData->Player[CurPlayer].fFriendlyFireDamageDealt = pMR->m_fFriendlyFireDamageDealt;
					SaveData->Player[CurPlayer].fDamageTaken = pMR->m_fDamageTaken;
					SaveData->Player[CurPlayer].iAliensKilled = pMR->m_iAliensKilled;
					SaveData->Player[CurPlayer].iMineKills = pMR->m_iMineKills;
					SaveData->Player[CurPlayer].iKiller = pMR->m_iKiller;
					SaveData->Player[CurPlayer].iSuicide = pMR->m_iSuicide;
					SaveData->Player[CurPlayer].iShieldbugsKilled = pMR->m_iShieldbugsKilled;
					SaveData->Player[CurPlayer].iCuredInfestation = pMR->m_iCuredInfestation;

					SaveData->Player[CurPlayer].iFastComputerHacks = pMR->m_iFastComputerHacks;
					SaveData->Player[CurPlayer].iFastDoorHacks = pMR->m_iFastDoorHacks;
					SaveData->Player[CurPlayer].iNoAdrenFastDoorHacks = pMR->m_iNoAdrenFastDoorHacks;
					SaveData->Player[CurPlayer].iParasitesKilledByChainsaw = pMR->m_iParasitesKilledByChainsaw;

					SaveData->Player[CurPlayer].iMedicHealing = pMR->m_iMedicHealing;
					SaveData->Player[CurPlayer].iParasitesKilled = pMR->m_iParasitesKilled;
					SaveData->Player[CurPlayer].iSentryKills = pMR->m_iSentryKills;
					SaveData->Player[CurPlayer].iEggKills = pMR->m_iEggKills;


					//Awesome Team - расчет 40%
					CASW_Marine *pMar = pMR->GetMarineEntity();
					if (pMar) {
						float max_health = pMar->GetMaxHealth();
						float perc = pMR->m_fDamageTaken * 100 / max_health;
						if (perc >= 40) //40%
							SaveData->bAwesome = false;
					} else SaveData->bAwesome = false;

					//считаем количество реально живых клиентов
					SaveData->Player[CurPlayer].bAlive = true;
					if (pMR->GetHealthPercent() <= 0) {
						SaveData->bAllMarinesAlive = false; // если хотябы один марин помер во время миссии. общий статус живы ли все или нет меняем на "НЕ ВСЕ ЖИВЫ".
						SaveData->Player[CurPlayer].bAlive = false;
					}

					//Проверяем урон для Perfect
					if (!pMR->m_bHurt)
						SaveData->Player[CurPlayer].bPerfect = true;
					else {
						SaveData->bPerfectTeam = false;
						SaveData->Player[CurPlayer].bPerfect = false;
					}
					//проверка: какие классы участвовали в игре
					if ((Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_jaeger") == 0) || (Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_sarge") == 0))
						OffClass++;
					if ((Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_vegas") == 0) || (Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_crash") == 0))
						TechClass++;
					if ((Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_bastille") == 0) || (Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_faith") == 0))
						MedicClass++;
					if ((Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_wolfe") == 0) || (Q_stricmp(SaveData->Player[CurPlayer].cProfile, "#asw_name_wildcat") == 0))
						GunClass++;
					

					//проверка - юзался ли фламер
					if (pMR->m_iFlamerFired <= 0) { // тут можно добавить один параметр для расшифроки каждого марина по фламеру
						//
					} else SaveData->bFlamerUsed = true;

					if (pMR->m_bTeslaGunUsed)
						SaveData->bTeslaGunUsed = true;

				} //end of loop players

				if (SaveData->maxClients >= 4 && SaveData->bAllMarinesAlive)
					SaveData->bAllMarinesAlive = true;
				else
					SaveData->bAllMarinesAlive = false;

				if (OffClass == 1 && TechClass == 1 && MedicClass == 1 && GunClass == 1)
					SaveData->bHasAllClasses = true;

				//PerfectTeam
				if ((SaveData->bAllMarinesAlive) && (SaveData->bPerfectTeam))
					SaveData->bPerfectTeam = true;
				else
					SaveData->bPerfectTeam = false;
				...
				char buftime[80];
				time_t long_time;
				time_t seconds = time(&long_time);
				//tm* timeinfo = localtime(&seconds);
				tm* gmttimeinfo = gmtime(&seconds);
				char* format = "%Y%m%d%H%M%S"; //for MySQL timestamp
				strftime(buftime, 80, format, gmttimeinfo);
				strcpy (SaveData->cMissionEndTime, buftime); //buftime - время когда была закончена попытка игры на карте.
			
				//ThreadExecute(SaveBaseData,SaveData);
				//CreateSimpleThread(SaveBaseData,SaveData);
				
				//Msg("*** SaveData: %d   tMap: %d\n", SaveData->missions_num, CurrentMap.missions_num);
				
				
				(HANDLE)_beginthreadex(NULL, 0, &SaveBaseData, SaveData, 0, NULL); //Запуск потока


				//Q_snprintf(buffer, sizeof(buffer), "%s: NEWNEW\n", pInf->GetPlayerName());
				//engine->ServerCommand( UTIL_VarArgs( "say 'SUPER NEW ACH'\n") ); // выдает сообщение с надписью Console: ХХХ
	...
	//Msg("***IsCampaignGame()=%d=====CampaignMissionsLeft()=%d\n", ASWGameRules()->IsCampaignGame(), ASWGameRules()->CampaignMissionsLeft());
...
void CAlienSwarm::OnChangeOnslaught( bool bVar )
{
	if ( !IsOnslaught() )
	{
		UTIL_ClientPrintAll( HUD_PRINTTALK, "Disabling Onslaught is not allowed on this server\n" );
		//asw_horde_override.SetValue(1);
		//asw_wanderer_override.SetValue(1);
		engine->ServerCommand("asw_horde_override 1\n");
		engine->ServerCommand("asw_wanderer_override 1\n");
	}
	if ( !IsHardcoreFF() )
	{
		UTIL_ClientPrintAll( HUD_PRINTTALK, "Disabling HardcoreFF is not allowed on this server\n" );
		//asw_marine_ff_absorption.SetValue(0);
		//asw_sentry_friendly_fire_scale.SetValue(1);
		engine->ServerCommand("asw_marine_ff_absorption 0\n");
		engine->ServerCommand("asw_sentry_friendly_fire_scale 1\n");
	}
}
...
void CAlienSwarm::OnSkillLevelChanged( int iNewLevel )
{
	...
	//Zak - смена сложности (change level change diff)
	//Меняем сложность. на будущее - надо перенести настройки в базу
	...
	extern ConVar asw_marine_burn_time_easy;
	extern ConVar asw_marine_burn_time_normal;
	extern ConVar asw_marine_burn_time_hard;
	extern ConVar asw_marine_burn_time_insane;
	
	extern ConVar asw_drone_acceleration;
	
	extern ConVar asw_marine_time_until_ignite;
	extern ConVar asw_horde_size_min;
	extern ConVar asw_horde_size_max;
	extern ConVar asw_horde_interval_min;
	extern ConVar asw_horde_interval_max;
	extern ConVar asw_director_peak_min_time;
	extern ConVar asw_director_peak_max_time;
	extern ConVar asw_director_relaxed_min_time;
	extern ConVar asw_director_relaxed_max_time;
	
	float fTemp = 0;

	fTemp = 0.2; asw_difficulty_alien_health_step.SetValue(fTemp);
	fTemp = 0.2; asw_difficulty_alien_damage_step.SetValue(fTemp);
	fTemp = 0; asw_marine_time_until_ignite.SetValue(fTemp);

	fTemp = 60;
	asw_marine_burn_time_easy.SetValue(fTemp);
	asw_marine_burn_time_normal.SetValue(fTemp);
	asw_marine_burn_time_hard.SetValue(fTemp);
	asw_marine_burn_time_insane.SetValue(fTemp);
	...
	switch (iNewLevel) {
		case 1 : //easy
			fTemp = 0.96; asw_marine_speed_scale_easy.SetValue(fTemp);
			fTemp = 0.7; asw_alien_speed_scale_easy.SetValue(fTemp);
			fTemp = 5; asw_drone_acceleration.SetValue(fTemp);

			fTemp = 20; asw_horde_size_min.SetValue(fTemp);
			fTemp = 30; asw_horde_size_max.SetValue(fTemp);
			
			fTemp = 10; asw_horde_interval_min.SetValue(fTemp);
			fTemp = 30; asw_horde_interval_max.SetValue(fTemp);

			fTemp = 2; asw_director_peak_min_time.SetValue(fTemp);
			fTemp = 4; asw_director_peak_max_time.SetValue(fTemp);
			
			fTemp = 15; asw_director_relaxed_min_time.SetValue(fTemp);
			fTemp = 30; asw_director_relaxed_max_time.SetValue(fTemp);
			break;
		case 2 : //normal
			fTemp = 1.0; asw_marine_speed_scale_normal.SetValue(fTemp);
			fTemp = 1.0; asw_alien_speed_scale_normal.SetValue(fTemp);
			fTemp = 5; asw_drone_acceleration.SetValue(fTemp);

			fTemp = 9; asw_horde_size_min.SetValue(fTemp);
			fTemp = 30; asw_horde_size_max.SetValue(fTemp);
			
			fTemp = 15; asw_horde_interval_min.SetValue(fTemp);
			fTemp = 60; asw_horde_interval_max.SetValue(fTemp);
			...
			break;
		case 3 : //hard
			fTemp = 1.024; asw_marine_speed_scale_hard.SetValue(fTemp);
			fTemp = 1.7; asw_alien_speed_scale_hard.SetValue(fTemp);
			fTemp = 8; asw_drone_acceleration.SetValue(fTemp);

			fTemp = 9; asw_horde_size_min.SetValue(fTemp);
			fTemp = 30; asw_horde_size_max.SetValue(fTemp);
			
			fTemp = 15; asw_horde_interval_min.SetValue(fTemp);
			fTemp = 120; asw_horde_interval_max.SetValue(fTemp);
			...
			break;
		case 4 : //insane
			fTemp = 1.048; asw_marine_speed_scale_insane.SetValue(fTemp);
			fTemp = 1.8; asw_alien_speed_scale_insane.SetValue(fTemp);
			fTemp = 9; asw_drone_acceleration.SetValue(fTemp);
			
			fTemp = 9; asw_horde_size_min.SetValue(fTemp);
			fTemp = 30; asw_horde_size_max.SetValue(fTemp);
			
			fTemp = 15; asw_horde_interval_min.SetValue(fTemp);
			fTemp = 80; asw_horde_interval_max.SetValue(fTemp);
		...
		case 5 : //brutal
			fTemp = 1.048; asw_marine_speed_scale_insane.SetValue(fTemp);
			fTemp = 1.9; asw_alien_speed_scale_insane.SetValue(fTemp);
			fTemp = 10; asw_drone_acceleration.SetValue(fTemp);
			...
			fTemp = 10; asw_director_relaxed_min_time.SetValue(fTemp);
			fTemp = 30; asw_director_relaxed_max_time.SetValue(fTemp);
			break;
	}
	...
	/*	if (iNewLevel == 1)	//  convert easy to normal
	{
		m_iMissionDifficulty = 5;
		szDifficulty = "normal";
		m_iSkillLevel = 2; // normal
		iNewLevel = 2;
		ConVar *var = (ConVar *)cvar->FindVar( "asw_skill" );
		if (var) { var->SetValue(m_iSkillLevel); }
		UTIL_ClientPrintAll( HUD_PRINTTALK, "Easy is not allowed on this server" );
	}
*/
		...
		case ASW_FR_CAMPAIGN_MAP:
			{
				...
				for (int i=0;i<ASW_MAX_READY_PLAYERS;i++)
				{
					//Zak - автореди, не знаю тут зачем
					//pGameResource->m_bPlayerReady.Set(i, false);
					pGameResource->m_bPlayerReady.Set(i, true);
...
/*
unsigned int __stdcall UpdateIt (void*)
{
	//CurrentNewView = false;
	return 0;
}
*/

/*
unsigned int __stdcall ShowWelcome (void* pParam)
//void ShowWelcome (void* pParam)
{
	...
	//cWelcome* par = (cWelcome*) pParam; // always use a struct!
	//class CASW_Player;
	//CASW_Player *pPlayer = (CASW_Player *) pParam;
	//CBasePlayer *pPlayer = dynamic_cast(CBasePlayer *) pParam;
	//CBasePlayer *player = ToBasePlayer( GetOwnerEntity() );
	CBasePlayer *pPlayer = static_cast<CBasePlayer *>( pParam );
	//CASW_Player *pPlayer = dynamic_cast<CASW_Player*>(UTIL_PlayerByIndex(par->Index));
	if ( !pPlayer )
		return 0;
	...
	float BS = gpGlobals->curtime;
	
	srand((unsigned)time(NULL));
	//from 1980 to 2020
	int u = (double)rand() / (RAND_MAX + 1) * (2020 - 1980) + 1980;
	//Sleep(u);
	Sleep(2);
	//if (gpGlobals->curtime > BS) {
	CBasePlayer *ppPlayer = static_cast<CBasePlayer *>( pParam );

*/
	/*
	if ( !ppPlayer )
			return 0;
		//ClientPrint(pPlayer, HUD_PRINTTALK, "Welcome to ASB2 MOD.\ntype !help in main chat for more details\n\nwww.google.ru - statistics server\n");

		extern void ClientPrint( CBasePlayer *player, int msg_dest, const char *msg_name, const char *param1 = NULL, const char *param2 = NULL, const char *param3 = NULL, const char *param4 = NULL );
		Q_snprintf(buffer, sizeof(buffer), "Welcome to ASB2 MOD.\ntype !help in main chat for more details\nasb2.org - statistics server\nYou have: %d pts.\n", pPoints);
		
		ClientPrint(ppPlayer, HUD_PRINTTALK, buffer);

		if (bLockedMap)
			ClientPrint(ppPlayer, HUD_PRINTTALK, "Unknown map!\n");
		if (bStatError)
			ClientPrint(ppPlayer, HUD_PRINTTALK, NOSTAT);
		if (bNewVersion)
			myMessage("NEW VERSION DETECTED! PLEASE UPDATE THIS SERVER");
	*/
	/*
...
void CAlienSwarm::OnPlayerFullyJoined( CASW_Player *pPlayer )
{
	...
	//zak - autokick авто кик

	if ( pPlayer ) {
		
		if (pPlayer->entindex()) {

			int Prom = pPlayer->GetPromotion();
			int XP = pPlayer->GetExperience();
			if ((XP < 42250) && (Prom == 0)) {
				char nam[50];
				strcpy (nam, pPlayer->GetPlayerName());
				engine->ServerCommand (CFmtStr("kickid %s {Players below level 27 with no promotion cannot play ASB2 Difficulty Mod, Please Level Up!}\n", pPlayer->GetASWNetworkID()));
						
				int lvl = 0;
				if (XP >= 0) lvl = 1; if (XP >= 1000) lvl = 2; if (XP >= 2050) lvl = 3; if (XP >= 3150) lvl = 4; if (XP >= 4300) lvl = 5; if (XP >= 5500) lvl = 6;
				if (XP >= 6750) lvl = 6; if (XP >= 8050) lvl = 8; if (XP >= 9400) lvl = 9; if (XP >= 10800) lvl = 10; if (XP >= 12250) lvl = 11; if (XP >= 13750) lvl = 12;
				if (XP >= 15300) lvl = 13; if (XP >= 16900) lvl = 14; if (XP >= 18550) lvl = 15; if (XP >= 20250) lvl = 16; if (XP >= 22000) lvl = 17; if (XP >= 23800) lvl = 18;
				if (XP >= 25650) lvl = 19; if (XP >= 27550) lvl = 20; if (XP >= 29500) lvl = 21; if (XP >= 31500) lvl = 22; if (XP >= 33550) lvl = 23; if (XP >= 35650) lvl = 24;
				if (XP >= 37800) lvl = 25; if (XP >= 40000) lvl = 26;

				char text[128];
				Q_snprintf(text, sizeof(text), "%s was kicked (prom %d, lvl %d)\n", nam, Prom, lvl);
				UTIL_ClientPrintAll(HUD_PRINTTALK, text);
				Msg(text);
			}
			else
				if ( pPlayer ) {
					int idx = pPlayer->GetClientIndex()+1;
					strcpy (PlayerData[idx].cSteamId, pPlayer->GetASWNetworkID());
				}
		}
	}
